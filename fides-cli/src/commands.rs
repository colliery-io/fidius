...
fn check_crates_io(name: &str) -> Option<String> {
    ...
    let mut response = ureq::get(&url)
        .header("User-Agent", "fides-cli (https://github.com/fides-rs/fides)")
        .call()
        .ok()?;
...
pub fn init_interface(
    ...
    // Resolve fides dependency
    let fides_dep = resolve_dep("fides", version);
...
"#
    ...
    let lib_rs = format!(
        r#"pub use fides::{{plugin_impl, PluginError}};
...
pub fn inspect(dylib_path: &Path) -> Result {
    let loaded = fides_host::loader::load_library(dylib_path)
