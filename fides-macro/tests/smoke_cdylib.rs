...
fn load_cdylib_and_call_plugin() {
    ...
    // Get the registry via fides_get_registry function
    let get_registry: libloading::Symbol<unsafe extern "C" fn() -> *const fides_core::descriptor::PluginRegistry> =
        unsafe { lib.get(b"fides_get_registry") }
            .expect("failed to find fides_get_registry symbol");

    let registry = unsafe { &*get_registry() };

    // Verify registry
    assert_eq!(&registry.magic, b"FIDES\0\0\0");
    ...
    let output: AddOutput = fides_core::wire::deserialize(output_slice).unwrap();
