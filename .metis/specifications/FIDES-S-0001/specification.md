---
id: fides-abi-and-wire-protocol
level: specification
title: "Fides ABI and Wire Protocol Specification"
short_code: "FIDES-S-0001"
...
- **Plugin author**: Implements the trait with `#[plugin_impl]`. Depends only on the interface crate (which re-exports fides macros and types).
- **Host application**: Loads dylibs, validates descriptors, calls plugin methods through generated proxies. Depends on the interface crate + `fides-host`.
...
```
fides/
├── fides-core/       # Shared types: descriptor layout, buffer strategy traits, wire format,
│                     # capability bits, version hashes, error types, signing types
├── fides-macro/      # proc-macro: #[plugin_interface], #[plugin_impl]
├── fides-host/       # Host-side: loading, validation, signature verification, calling
├── fides-cli/        # CLI: init-interface, init-plugin, sign, verify, inspect
└── fides/            # Facade crate re-exporting core + macro
```

`fides-core` is the only shared library dependency. The `fides` facade re-exports `fides-core` types and `fides-macro` macros so that interface crates have a single dependency.

## Developer Workflow

### Interface Crate (The Schema)

The interface author creates a dedicated crate that defines the plugin contract. This crate is the single dependency for plugin authors — it re-exports all necessary fides macros and types.

**Scaffolding via CLI:**

```
$ cargo install fides-cli
$ fides init-interface my-app-plugin-api --trait ImageFilter
```
...
```
$ fides init-plugin my-blur-plugin --interface my-app-plugin-api
```
...
│   └── fides (facade → fides-core + fides-macro)
└── fides-host (loading, validation, calling)
    └── fides-core
...
Plugin authors never depend on `fides` directly — the interface crate is their entire contract surface.
...
#[no_mangle]
pub static FIDES_PLUGIN_REGISTRY: PluginRegistry = ...;
```

The host reads `FIDES_PLUGIN_REGISTRY` first, then iterates over the descriptor array. Single-plugin dylibs still use the registry — with `plugin_count = 1`.
...
```
dlopen(path)
  → dlsym("FIDES_PLUGIN_REGISTRY")           // no code execution
