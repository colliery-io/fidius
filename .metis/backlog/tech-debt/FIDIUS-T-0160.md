---
id: deep-user-typed-tuple-nesting-in
level: task
...
# Deep user-typed tuple nesting in WASM arg marshalling (tuple-in-record)
...
PC.1 ([[FIDIUS-T-0152]]) covers top-level and list-nested tuples/maps in WASM arg lowering, but a tuple **inside a record** (or `Vec<tuple-with-user-type>`) falls back to the structural path in `value_to_val_typed` (`crates/fidius-host/src/executor/wasm.rs`) instead of threading the wasmtime `Type` through record fields. Recurse the field types so user-typed tuples nested in records lower to `Val::Tuple`. Low priority; uncommon shape.
