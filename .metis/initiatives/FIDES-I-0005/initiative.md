---
id: fides-cli-developer-tooling-and
level: initiative
...
initiative_id: fides-cli-developer-tooling-and
---

# fides-cli — Developer Tooling and Signing

## Context

The CLI makes fides usable as a product, not just a library. `cargo install fides-cli` gives developers everything they need: scaffold an interface crate, scaffold a plugin crate, generate signing keys, sign plugins, verify signatures, and inspect compiled dylibs. Without it, developers have to manually set up the crate topology, which is error-prone.

Depends on FIDES-I-0001 (core types for inspect), FIDES-I-0003 (host loading for inspect), and conceptually on FIDES-I-0004 (the facade crate that scaffolded projects depend on).

## Goals & Non-Goals

**Goals:**
- `fides init-interface <name> --trait <TraitName>` — scaffold an interface crate with correct Cargo.toml (depends on `fides`) and lib.rs (re-exports + annotated trait stub)
...
```
<name>/
├── Cargo.toml     # [dependencies] fides = "0.1"
└── src/
...
Loads the dylib via fides-host's registry loading (without calling any plugin methods), displays:
```
...
- `fides-host` — for inspect (registry loading)
...
7. Implement `inspect` command (depends on fides-host loading)
