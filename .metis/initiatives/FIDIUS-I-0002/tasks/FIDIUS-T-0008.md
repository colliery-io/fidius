...
- [ ] Each shim: deserializes input via `fidius_core::wire::deserialize`, calls the real method on a static instance, serializes output via `fidius_core::wire::serialize`, returns status code
...
- [ ] For single-plugin case: emits `#[no_mangle] pub static FIDIUS_PLUGIN_REGISTRY: PluginRegistry` pointing to the descriptor
...
File: `fidius-macro/src/impl_macro.rs`
...
unsafe extern "C" fn __fides_blur_process(
    ...
    std::panic::catch_unwind(|| {
        let input = fidius_core::wire::deserialize(slice::from_raw_parts(in_ptr, in_len as usize))?;
        let result = INSTANCE.process(input);
        // handle Result<T, PluginError>
        let output_bytes = fidius_core::wire::serialize(&result)?;
...
### Dependencies
- FIDIUS-T-0006 (IR), FIDIUS-T-0007 (vtable struct + constants exist)

## Status Updates

- **2026-03-29**: Implemented in `fidius-macro/src/impl_macro.rs`. Generates: extern "C" shims with catch_unwind + wire serialize/deserialize, static instance, vtable static, descriptor static via builder fn, free_buffer fn, single-plugin FIDIUS_PLUGIN_REGISTRY. Added DescriptorPtr wrapper to fidius-core for Sync safety. 3 integration tests pass including full FFI round-trip (serialize input → call vtable → deserialize output → verify "Hello, World!").