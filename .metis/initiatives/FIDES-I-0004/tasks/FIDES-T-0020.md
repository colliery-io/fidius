...
Write the comprehensive E2E test suite that proves the full fides pipeline works. This extends the existing fides-host integration tests (I-0003) with additional scenarios: signed plugins, multi-plugin via fides-host, and negative tests (bad signature, not-found). Uses the existing test-plugin-smoke cdylib.
...
- [ ] Multi-plugin via fides-host: extend test-plugin-smoke with two impls, load both via `discover()`, call both
...
File: `fides-host/tests/e2e.rs` or extend `fides-host/tests/integration.rs`.
...
### Dependencies
- FIDES-T-0019 (facade must be complete)

## Status Updates

- **2026-03-29**: Wired signing::verify_signature into PluginHost::load(). 4 E2E tests in fides-host/tests/e2e.rs: signed+correct key succeeds, signed+wrong key fails (SignatureInvalid), unsigned+required fails (SignatureRequired), unsigned+not-required succeeds and calls method. Added Debug impl for LoadedPlugin. 56 tests pass across full workspace.