...
### Dependencies
- FIDIUS-T-0001 (workspace with serde, serde_json, bincode deps)

## Status Updates

- **2026-03-29**: Implemented in `fidius-core/src/wire.rs`. `WireError` uses thiserror with `From` impls for both serde_json and bincode errors. `WIRE_FORMAT` constant and serialize/deserialize functions cfg-gated. Compiles clean in dev profile.