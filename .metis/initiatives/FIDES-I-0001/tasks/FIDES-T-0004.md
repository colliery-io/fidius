...
Files:
- `fides-core/src/error.rs` — `PluginError`, `WireError` (if not already in wire.rs)
- `fides-core/src/status.rs` — status code constants
- `fides-core/src/hash.rs` — FNV-1a implementation
