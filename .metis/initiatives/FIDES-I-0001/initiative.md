---
id: fides-core-foundation-types
level: initiative
...
initiative_id: fides-core-foundation-types
---

# fides-core — Foundation Types

## Context

fides-core is the shared vocabulary crate that both plugin and host sides depend on. It defines the `#[repr(C)]` types that cross the FFI boundary, the buffer strategy abstraction, wire format serialization, error types, and status codes. Everything in the spec's "Plugin Descriptor" and "Wire Format" sections lands here as concrete Rust types.

This is the first crate to build because fides-macro and fides-host both depend on it. Getting these types right is critical — they become the stable ABI contract.
...
- Establish the Cargo workspace for the entire fides project

**Non-Goals:**
- Proc macro implementation (FIDES-I-0002)
- Dynamic loading logic (FIDES-I-0003)
- CLI tooling (FIDES-I-0005)
...
Initialize a Cargo workspace at the repo root with members for all five crates. Only fides-core gets real code in this initiative; the others are empty stubs to establish the dependency graph.
