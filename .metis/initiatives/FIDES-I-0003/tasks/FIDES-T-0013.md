...
Define the error and metadata types that fides-host uses. `LoadError` is the enum returned by all loading/validation operations — one variant per failure mode. `PluginInfo` is the owned metadata struct copied from FFI descriptor data (no raw pointers). `LoadPolicy` controls strictness.
...
File: `fides-host/src/error.rs` for LoadError, `fides-host/src/types.rs` for PluginInfo and LoadPolicy.
...
- **2026-03-29**: Implemented. `LoadError` (12 variants + Io), `CallError` (6 variants), `PluginInfo`, `LoadPolicy` all in fides-host. Added thiserror/serde/serde_json deps. Compiles clean.