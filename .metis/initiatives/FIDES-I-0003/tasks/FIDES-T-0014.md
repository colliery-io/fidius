...
Implement the core loading function: `dlopen` a dylib, `dlsym("fides_get_registry")`, call it, validate the registry and each descriptor, copy all FFI data to owned types. This is the heart of fides-host — the full validation sequence from the spec.
...
- [ ] `load_library(path) -> Result<(Arc<Library>, PluginRegistry), LoadError>` — dlopen + dlsym + call fides_get_registry
...
File: `fides-host/src/loader.rs`

Key change from spec: the host calls `dlsym("fides_get_registry")` which returns a `extern "C" fn() -> *const PluginRegistry` (changed in FIDES-I-0002). Not a static symbol.
