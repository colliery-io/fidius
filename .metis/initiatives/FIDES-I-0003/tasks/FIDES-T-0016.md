...
### Dependencies
- FIDES-T-0013 (types), FIDES-T-0014 (loader provides Arc<Library>)

## Status Updates

- **2026-03-29**: Implemented in `fides-host/src/handle.rs`. `call_method<I,O>(index, input)` handles full lifecycle: serialize → FFI call → status check → deserialize → free_buffer. Handles all status codes including STATUS_PLUGIN_ERROR (deserializes PluginError from output). `has_capability()`, `info()`, `from_loaded()`. Send+Sync. Compiles clean.