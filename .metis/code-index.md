...
- pub `client_stream` module L44 — `-` — Client-streaming guest consumer (`WasmHostStream`) over the `fidius:stream-pull`
- pub `python_descriptor` module L45 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `status` module L46 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `stream_ffi` module L47 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `stream_marker` module L48 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `value` module L49 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `wasm_descriptor` module L50 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `wire` module L51 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
...
- pub `call_bidi_streaming` function L136-151 — `( &self, method: usize, items: Vec<Value>, args: Value, ) -> Result<crate::strea...` — Bidirectional streaming (FIDIUS-I-0032 / ADR-0010): the host produces `items`
...
-  `Pyo3Executor` type L111-128 — `= Pyo3Executor` — routed through the neutral `Value` currency.
-  `call_streaming` function L112-127 — `( &self, method: usize, args: Value, ) -> Result<crate::stream::ChunkStream, Cal...` — routed through the neutral `Value` currency.
-  `Pyo3Executor` type L131-152 — `= Pyo3Executor` — routed through the neutral `Value` currency.
-  `pump_python_stream` function L159-198 — `(stream: fidius_python::PythonStream) -> crate::stream::ChunkStream` — Pump a `PythonStream` (a guest generator) into a [`crate::stream::ChunkStream`] on a
...
- pub `WasmMethod` struct L380-388 — `{ name: String, wire_raw: bool, streaming: bool }` — A method on the WASM interface, in declaration (vtable) order.
- pub `WasmComponentExecutor` struct L391-420 — `{ engine: Engine, instance_pre: InstancePre<HostState>, interface: String, metho...` — WASM component execution backend.
- pub `from_component_bytes` function L431-439 — `( bytes: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Build an executor from raw component bytes (a `.wasm` component).
- pub `from_component_bytes_with_egress` function L444-467 — `( bytes: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Like [`Self::from_component_bytes`] but with an embedder [`EgressPolicy`]
...
-  `HostState` struct L123-132 — `{ ctx: WasiCtx, table: ResourceTable, http_ctx: WasiHttpCtx, hooks: EgressHooks,...` — Per-store host state.
-  `HostState` type L134-142 — `impl WasiHttpView for HostState` — from the package manifest's allow-list.
-  `http` function L135-141 — `(&mut self) -> WasiHttpCtxView<'_>` — from the package manifest's allow-list.
-  `KNOWN_CAPABILITIES` variable L150-159 — `: &[&str]` — Capabilities the host knows how to grant.
-  `validate_capabilities` function L163-225 — `(caps: &[String]) -> Result<(), CallError>` — Reject unknown capability names early (at load) so a typo fails closed and
-  `build_wasi_ctx` function L231-292 — `(caps: &[String]) -> WasiCtx` — Build a `WasiCtx` from the allow-list.
-  `is_blocked_ip` function L299-318 — `(ip: &IpAddr) -> bool` — Baseline SSRF denylist for the raw-socket grant (FIDIUS-T-0143): an address a
-  `HOST_WASI_HTTP` variable L323 — `: (u32, u32, u32)` — The `wasi:http` version this host provides — matched to `wasmtime-wasi-http`
-  `wasi_http_incompatibility` function L335-365 — `(import_names: impl Iterator<Item = &'a str>) -> Option<String>` — Scan a component's import names for a `wasi:http` version this host can't
...
-  `tests` module L598-794 — `-` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
...
- pub `call_bidi_streaming_start` function L220-273 — `( &self, method_index: usize, items_json: &[u8], args_json: &[u8], ) -> Result<c...` — **Bidirectional** streaming (FIDIUS-I-0032 / ADR-0010): the method receives the
- pub `call_streaming_start` function L279-304 — `( &self, method_index: usize, input_json: &[u8], ) -> Result<crate::stream::Pyth...` — Start a server-streaming call (FIDIUS-I-0026).
- pub `call_raw` function L307-326 — `(&self, method_index: usize, input: &[u8]) -> Result<Vec<u8>, PythonCallError>` — Raw dispatch — pass bytes in, get bytes out, no encoding.
-  `PythonPluginHandle` type L84-351 — `= PythonPluginHandle` — `code = <ExceptionClassName>` otherwise.
...
-  `lookup_method` function L328-350 — `( &self, index: usize, attempting_raw: bool, ) -> Result<MethodLookup<'_>, Pytho...` — `code = <ExceptionClassName>` otherwise.
-  `MethodLookup` struct L353-355 — `{ callable: &'a Py<PyAny> }` — `code = <ExceptionClassName>` otherwise.
-  `build_call_args` function L364-383 — `( py: Python<'py>, input: &serde_json::Value, ) -> PyResult<Bound<'py, PyTuple>>` — Build positional args for `callable.call(...)` from a JSON value.
-  `HostFedStream` struct L389-391 — `{ items: std::vec::IntoIter<serde_json::Value> }` — A host-fed Python iterator (FIDIUS-I-0030 CS2.4): yields the host's stream items
-  `HostFedStream` type L394-405 — `= HostFedStream` — `code = <ExceptionClassName>` otherwise.
-  `__iter__` function L395-397 — `(slf: PyRef<'_, Self>) -> PyRef<'_, Self>` — `code = <ExceptionClassName>` otherwise.
-  `__next__` function L399-404 — `(&mut self, py: Python<'_>) -> PyResult<Option<Py<PyAny>>>` — `code = <ExceptionClassName>` otherwise.
