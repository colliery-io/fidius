...
> Generated: 2026-03-29T11:25:53Z | 33 files | Rust
...
- pub `PluginHost` struct L13-21 — `{ search_paths: Vec<PathBuf>, load_policy: LoadPolicy, require_signature: bool, ...` — Host for loading and managing plugins.
- pub `PluginHostBuilder` struct L24-32 — `{ search_paths: Vec<PathBuf>, load_policy: LoadPolicy, require_signature: bool, ...` — Builder for configuring a PluginHost.
- pub `search_path` function L48-51 — `(mut self, path: impl Into<PathBuf>) -> Self` — Add a directory to search for plugin dylibs.
- pub `load_policy` function L54-57 — `(mut self, policy: LoadPolicy) -> Self` — Set the load policy (Strict or Lenient).
- pub `require_signature` function L60-63 — `(mut self, require: bool) -> Self` — Require plugins to have valid signatures.
- pub `trusted_keys` function L66-69 — `(mut self, keys: &[VerifyingKey]) -> Self` — Set trusted Ed25519 public keys for signature verification.
- pub `interface_hash` function L72-75 — `(mut self, hash: u64) -> Self` — Set the expected interface hash for validation.
- pub `wire_format` function L78-81 — `(mut self, format: WireFormat) -> Self` — Set the expected wire format for validation.
- pub `buffer_strategy` function L84-87 — `(mut self, strategy: BufferStrategyKind) -> Self` — Set the expected buffer strategy for validation.
- pub `build` function L90-100 — `(self) -> Result<PluginHost, LoadError>` — Build the PluginHost.
- pub `builder` function L105-107 — `() -> PluginHostBuilder` — Create a new builder.
- pub `discover` function L113-152 — `(&self) -> Result<Vec<PluginInfo>, LoadError>` — Discover all valid plugins in the configured search paths.
- pub `load` function L158-195 — `(&self, name: &str) -> Result<LoadedPlugin, LoadError>` — Load a specific plugin by name.
-  `PluginHostBuilder` type L34-101 — `= PluginHostBuilder` — PluginHost builder and plugin discovery.
-  `new` function L35-45 — `() -> Self` — PluginHost builder and plugin discovery.
-  `PluginHost` type L103-196 — `= PluginHost` — PluginHost builder and plugin discovery.
-  `is_dylib` function L199-208 — `(path: &Path) -> bool` — Check if a path has a platform-appropriate dylib extension.
...
- pub `load_library` function L42-89 — `(path: &Path) -> Result<LoadedLibrary, LoadError>` — Load a plugin library from a path.
- pub `validate_against_interface` function L127-161 — `( plugin: &LoadedPlugin, expected_hash: Option<u64>, expected_wire: Option<WireF...` — Validate a loaded plugin against expected interface parameters.
...
-  `validate_descriptor` function L92-124 — `( desc: &PluginDescriptor, library: &Arc<Library>, ) -> Result<LoadedPlugin, Loa...` — Validate a single descriptor and copy to owned types.
