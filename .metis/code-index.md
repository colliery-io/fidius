...
> Generated: 2026-09-03T20:26:30Z | 196 files | Go, JavaScript, Python, Rust
...
│   │       ├── configured_cdylib_dynamic_e2e.rs
...
│   │       ├── e2e.rs
│   │       ├── host_functions_e2e.rs
│   │       ├── host_functions_in_process.rs
│   │       ├── host_functions_unbound.rs
│   │       ├── hostname_egress_e2e.rs
│   │       ├── integration.rs
│   │       ├── keyword_fields_wasm.rs
│   │       ├── macro_egress_e2e.rs
│   │       ├── macro_wasm.rs
...
│   │       ├── records_wasm.rs
│   │       ├── response_hook_e2e.rs
│   │       ├── tcp_egress_e2e.rs
│   │       ├── wasm_bidi_stream_e2e.rs
│   │       ├── wasm_client_stream_e2e.rs
│   │       ├── wasm_egress_e2e.rs
│   │       ├── wasm_executor.rs
│   │       ├── wasm_fs_e2e.rs
│   │       ├── wasm_grants_e2e.rs
│   │       ├── wasm_host_functions_e2e.rs
│   │       ├── wasm_record_stream_item.rs
│   │       ├── wasm_record_stream_user_types.rs
...
│   │   ├── 04_pipeline.rs
│   │   ├── 05_record_stream.rs
│   │   ├── 06_bidi_transform.rs
│   │   ├── 07_client_streaming.rs
│   │   └── 08_host_functions.rs
│   └── src/
│       └── lib.rs
├── python/
│   ├── fidius/
...
│       └── test_sdk.py
└── tests/
    ├── test-plugin-hostcall/
    │   └── src/
    │       └── lib.rs
    ├── test-plugin-py-bidi-stream/
    │   └── transformer.py
...
#### crates/fidius/src/lib.rs

-  `facade_host_surface` module L203-218 — `-` — by which fidius could grow first-class timeout semantics.
-  `names` function L204-217 — `( _host: &crate::PluginHost, _builder: &crate::PluginHostBuilder, _handle: &crat...` — by which fidius could grow first-class timeout semantics.
-  `facade_wasm_surface` module L222-231 — `-` — by which fidius could grow first-class timeout semantics.
-  `names` function L223-230 — `( _policy: &dyn crate::EgressPolicy, _denied: crate::EgressDenied, // the `http`...` — by which fidius could grow first-class timeout semantics.

### crates/fidius/tests

> *Semantic summary to be generated by AI agent.*

#### crates/fidius/tests/facade_core.rs

-  `wire_roundtrip_through_facade` function L15-20 — `()`
-  `value_bridge_through_facade` function L23-28 — `()`
-  `plugin_error_is_reexported_and_constructs` function L31-34 — `()`
-  `hashing_is_reexported` function L37-44 — `()`
-  `abi_constants_are_reexported` function L47-51 — `()`
-  `Greeter` interface L57-59 — `{ fn greet() }`
-  `plugin_interface_macro_generates_hash_through_facade` function L63-69 — `()`
-  `EchoHost` interface L74-76 — `{ fn echo() }`
-  `host_interface_macro_generates_through_facade` function L80-95 — `()`
-  `host_ffi_types_are_reexported` function L98-106 — `()`
-  `assert_exists` function L99 — `()`

#### crates/fidius/tests/facade_host.rs

-  `plugin_host_builds_through_facade` function L9-20 — `()`
-  `host_types_are_reexported` function L23-35 — `()`
-  `assert_exists` function L25 — `()`
-  `host_function_bind_gate_is_reachable_through_facade` function L38-52 — `()`

#### crates/fidius/tests/facade_streaming.rs

-  `streaming_types_are_reexported` function L12-21 — `()`
-  `assert_exists` function L13 — `()`
-  `_accepts_executor` function L17 — `()`

#### crates/fidius/tests/facade_wasm.rs

-  `HttpOnly` struct L14 — `-` — A policy that implements only the required `authorize` (HTTP) and leaves the TCP
-  `HttpOnly` type L15-19 — `impl EgressPolicy for HttpOnly`
-  `authorize` function L16-18 — `(&self, _parts: &mut http_types::request::Parts) -> Result<(), EgressDenied>`
-  `egress_tcp_and_udp_default_to_deny_through_facade` function L22-34 — `()`
-  `egress_denied_constructs_through_facade` function L37-40 — `()`
-  `WasmEchoHost` interface L47-49 — `{ fn echo() }`
-  `host_interface_wasm_bind_surface_resolves_through_facade` function L53-64 — `()`
...
#### crates/fidius-core/src/host_registry.rs

- pub `HostImportEntry` struct L30-32 — `{ descriptor: &'static HostImportDescriptor }` — A submitted host-import descriptor pointer, collected via `inventory`.
- pub `get_host_import_registry` function L55-58 — `() -> &'static HostImportRegistry` — Get or build the host-import registry (cached after first call).
-  `build_host_import_registry` function L37-52 — `() -> HostImportRegistry` — Build the host-import registry from all submitted descriptors.
-  `REGISTRY` variable L56 — `: std::sync::OnceLock<HostImportRegistry>` — identically (nothing to bind).

#### crates/fidius-core/src/lib.rs

- pub `host_registry` module L16 — `-`
- pub `package` module L17 — `-`
- pub `registry` module L18 — `-`
- pub `async_runtime` module L21 — `-`
...
-  `tests` module L780-1909 — `-` — host-defined schema type.
...
-  `header_with_runtime` function L1701-1710 — `(rt: Option<&str>) -> PackageHeader` — host-defined schema type.
-  `runtime_string_maps_to_each_variant` function L1713-1734 — `()` — host-defined schema type.
-  `unpack_entry_count_is_an_exact_boundary` function L1737-1772 — `()` — host-defined schema type.
-  `unpack_size_budget_is_an_exact_boundary` function L1775-1810 — `()` — host-defined schema type.
-  `unpack_zero_max_ratio_disables_ratio_check` function L1813-1831 — `()` — host-defined schema type.
-  `pack_collects_nested_non_excluded_dirs` function L1834-1855 — `()` — host-defined schema type.
-  `manifest_for` function L1861-1882 — `(runtime: Option<&str>, python: bool, wasm: bool) -> PackageManifest<()>` — host-defined schema type.
-  `validate_runtime_section_matrix` function L1885-1908 — `()` — host-defined schema type.
...
-  `fidius_plugin_registry` macro L74-86 — `-` — Emit the `fidius_get_registry` export function.
...
#### crates/fidius-guest/src/client_stream.rs

- pub `WasmHostStream` struct L49-51 — `{ _marker: PhantomData<fn() -> T> }` — Guest consumer of a host-produced stream — the client-streaming `Stream<T>`
- pub `new` function L55-59 — `() -> Self` — Construct a consumer over the host's `fidius:stream-pull` import.
-  `bindings` module L32-44 — `-` — `fidius_guest::http`'s wasi:http import (I-0028).
-  `default` function L63-65 — `() -> Self` — `fidius_guest::http`'s wasi:http import (I-0028).
-  `Item` type L69 — `= T` — `fidius_guest::http`'s wasi:http import (I-0028).
-  `next` function L70-75 — `(&mut self) -> Option<T>` — `fidius_guest::http`'s wasi:http import (I-0028).
...
-  `tests` module L105-180 — `-` — plugins compiled against a different interface.
...
-  `streaming_markers_are_distinct` function L145-169 — `()` — plugins compiled against a different interface.
-  `different_signatures_differ` function L172-179 — `()` — plugins compiled against a different interface.

#### crates/fidius-guest/src/host_call.rs

- pub `call` function L56-71 — `( interface: &'static str, expected_version: u32, expected_hash: u64, index: u32...` — Invoke a host function through the `fidius:host-call` import: bincode of
- pub `probe` function L76-89 — `( interface: &'static str, expected_version: u32, expected_hash: u64, ) -> Resul...` — Run the name/version/hash gate without dispatching a function (the
-  `bindings` module L34-50 — `-` — `fidius:stream-pull` import.

#### crates/fidius-guest/src/host_ffi.rs

- pub `HOST_IMPORTS_VERSION` variable L75 — `: u32` — Current layout version of the [`HostImportRegistry`] struct.
- pub `HOST_IMPORTS_SYMBOL` variable L81 — `: &str` — Name of the optional symbol a plugin dylib exports to advertise the host
- pub `BIND_OK` variable L87 — `: i32` — The table was validated and installed.
- pub `BIND_ERR_NULL` variable L89 — `: i32` — The table pointer was null.
- pub `BIND_ERR_ALREADY_BOUND` variable L91 — `: i32` — A table for this interface was already installed (bind is once-only).
- pub `BIND_ERR_HASH_MISMATCH` variable L94 — `: i32` — The table's `interface_hash` does not match what the plugin was built
- pub `BIND_ERR_VERSION_MISMATCH` variable L96 — `: i32` — The table's `interface_version` does not match what the plugin expects.
- pub `BIND_ERR_ABI` variable L98 — `: i32` — The table was built by an incompatible fidius ABI.
- pub `BIND_ERR_FN_COUNT` variable L100 — `: i32` — The table's `fn_count` does not match the interface's method count.
- pub `BIND_ERR_PANIC` variable L102 — `: i32` — The bind shim panicked (never expected; defensive).
- pub `BIND_ERR_WRONG_BACKEND` variable L105 — `: i32` — The handle's backend cannot accept this table (e.g.
- pub `HOST_CALL_PROBE_INDEX` variable L121 — `: u32` — Reserved dispatch index for the bind-probe: `call(..., PROBE, [])` runs
- pub `HOST_CALL_STATUS_NOT_BOUND` variable L124 — `: i32` — host-call status: no host interface with this name is bound.
- pub `HOST_CALL_STATUS_VERSION_MISMATCH` variable L127 — `: i32` — host-call status: the bound table's version differs from what the plugin
- pub `HOST_CALL_STATUS_HASH_MISMATCH` variable L130 — `: i32` — host-call status: the bound table's signature hash differs.
- pub `HostDispatchFn` type L137-138 — `= unsafe extern "C" fn(*mut c_void, u32, *const u8, u32, *mut *mut u8, *mut u32)...` — The dispatch signature of a host-function table: `(ctx, fn_index, in_ptr,
- pub `HostFunctionTable` struct L156-178 — `{ table_size: u32, abi_version: u32, interface_name: *const c_char, interface_ha...` — A set of host functions offered to a plugin, as a C-ABI table.
- pub `HostImportDescriptor` struct L191-205 — `{ descriptor_size: u32, abi_version: u32, interface_name: *const c_char, interfa...` — A plugin's declaration that it can consume a host interface.
- pub `HostImportRegistry` struct L214-223 — `{ magic: [u8; 8], registry_version: u32, import_count: u32, imports: *const *con...` — Registry of all host-import declarations in a plugin dylib, returned by
- pub `HostImportPtr` struct L232 — `-` — A `Sync` wrapper for a raw `HostImportDescriptor` pointer, for use in
- pub `HostTableCell` struct L245 — `-` — The once-only cell a plugin stores its bound [`HostFunctionTable`] in.
- pub `new` function L250-252 — `() -> Self` — An empty (unbound) cell, for `static` initialization.
- pub `bind` function L259-269 — `(&self, table: *const HostFunctionTable) -> i32` — Install `table` if the cell is empty.
- pub `get` function L272-277 — `(&self) -> Option<&HostFunctionTable>` — The bound table, or `None` if the host never bound this interface.
- pub `HostCallError` enum L287-343 — `NotBound | Host | HostPanic | Serialization | Deserialization | InvalidIndex | U...` — Error returned to plugin code when a host-function call fails.
- pub `decode_host_call_status` function L348-400 — `( interface: &'static str, index: u32, status: i32, payload: Vec<u8>, ) -> Resul...` — Decode a `(status, payload)` pair from the WASM `fidius:host-call` import
- pub `HostBindError` struct L406-413 — `{ interface: &'static str, code: i32, message: &'static str }` — Error installing a [`HostFunctionTable`] into a plugin — the typed form
- pub `bind_status_to_result` function L416-426 — `(interface: &'static str, code: i32) -> Result<(), HostBindError>` — Map a `BIND_*` status to a `Result` for host-side bind entry points.
- pub `bind_status_message` function L429-441 — `(code: i32) -> &'static str` — Map a `BIND_*` status to a human-readable description (for error paths).
- pub `host_callback_depth` function L452-454 — `() -> usize` — Per-thread depth of live host-function dispatches.
- pub `CallbackDepthGuard` struct L463 — `-` — RAII guard incrementing [`host_callback_depth`] — used by the generated
- pub `new` function L467-470 — `() -> Self` — taken at depth > 0).
- pub `call_host_fn` function L487-552 — `( table: &HostFunctionTable, index: u32, input: &[u8], ) -> Result<Vec<u8>, Host...` — Invoke a host function through a bound table: bincode-of-args in,
-  `HostFunctionTable` type L182 — `impl Send for HostFunctionTable` — taken at depth > 0).
-  `HostFunctionTable` type L183 — `impl Sync for HostFunctionTable` — taken at depth > 0).
-  `HostImportDescriptor` type L208 — `impl Send for HostImportDescriptor` — taken at depth > 0).
-  `HostImportDescriptor` type L209 — `impl Sync for HostImportDescriptor` — taken at depth > 0).
-  `HostImportRegistry` type L226 — `impl Send for HostImportRegistry` — taken at depth > 0).
-  `HostImportRegistry` type L227 — `impl Sync for HostImportRegistry` — taken at depth > 0).
-  `HostImportPtr` type L235 — `impl Send for HostImportPtr` — taken at depth > 0).
-  `HostImportPtr` type L236 — `impl Sync for HostImportPtr` — taken at depth > 0).
-  `HostTableCell` type L247-278 — `= HostTableCell` — taken at depth > 0).
-  `CallbackDepthGuard` type L465-471 — `= CallbackDepthGuard` — taken at depth > 0).
-  `CallbackDepthGuard` type L473-477 — `impl Drop for CallbackDepthGuard` — taken at depth > 0).
-  `drop` function L474-476 — `(&mut self)` — taken at depth > 0).
-  `tests` module L555-606 — `-` — taken at depth > 0).
-  `cell_binds_exactly_once` function L559-590 — `()` — taken at depth > 0).
-  `CELL` variable L560 — `: HostTableCell` — taken at depth > 0).
-  `dispatch` function L564-573 — `( _: *mut c_void, _: u32, _: *const u8, _: u32, _: *mut *mut u8, _: *mut u32, ) ...` — taken at depth > 0).
-  `free_buffer` function L574 — `(_: *mut u8, _: usize)` — taken at depth > 0).
-  `depth_guard_nests_and_unwinds` function L593-605 — `()` — taken at depth > 0).
...
- pub `host_ffi` module L39 — `-` — Host-function (plugin → host callback) FFI contract — the reverse
- pub `http` module L43 — `-` — Brokered outbound HTTP for sandboxed WASM connectors (FIDIUS-I-0028).
- pub `sockets` module L49 — `-` — Capability-gated outbound TCP for sandboxed WASM connectors (FIDIUS-I-0033) —
- pub `client_stream` module L54 — `-` — Client-streaming guest consumer (`WasmHostStream`) over the `fidius:stream-pull`
- pub `host_call` module L59 — `-` — Guest-side host-function calls over the `fidius:host-call` import — the
- pub `python_descriptor` module L60 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `status` module L61 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `stream_ffi` module L62 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `stream_marker` module L63 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `value` module L64 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `wasm_descriptor` module L65 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
- pub `wire` module L66 — `-` — (per ADR-0002), so `fidius-guest` is versioned in lockstep with `fidius-core`.
...
#### crates/fidius-guest/src/sockets.rs

- pub `tcp` module L65-211 — `-` — Blocking outbound TCP, the policy-gated counterpart of [`crate::http`].
- pub `TcpStream` struct L77-79 — `{ inner: StdTcpStream }` — A connected, blocking TCP stream.
- pub `connect` function L92-94 — `(host: &str, port: u16) -> io::Result<TcpStream>` — Open a TCP connection to `host:port`, blocking until connected.
- pub `connect_to` function L100-102 — `(addr: A) -> io::Result<Self>` — Connect to any [`ToSocketAddrs`] target — e.g.
- pub `connect_timeout` function L110-128 — `(host: &str, port: u16, timeout: Duration) -> io::Result<TcpStream>` — Connect with a bound on the **total** connect time, resolving `host`
- pub `set_read_timeout` function L132-134 — `(&self, dur: Option<Duration>) -> io::Result<()>` — Set the read timeout; `None` blocks indefinitely.
- pub `set_write_timeout` function L137-139 — `(&self, dur: Option<Duration>) -> io::Result<()>` — Set the write timeout; `None` blocks indefinitely.
- pub `set_nodelay` function L142-144 — `(&self, nodelay: bool) -> io::Result<()>` — Enable/disable `TCP_NODELAY` (disable Nagle's algorithm).
- pub `peer_addr` function L147-149 — `(&self) -> io::Result<SocketAddr>` — The remote peer's address.
- pub `local_addr` function L152-154 — `(&self) -> io::Result<SocketAddr>` — The local address this socket is bound to.
- pub `get_ref` function L158-160 — `(&self) -> &StdTcpStream` — Borrow the underlying [`std::net::TcpStream`] — an escape hatch for
- pub `get_mut` function L163-165 — `(&mut self) -> &mut StdTcpStream` — Mutably borrow the underlying [`std::net::TcpStream`].
- pub `into_std` function L168-170 — `(self) -> StdTcpStream` — Consume `self`, returning the underlying [`std::net::TcpStream`].
-  `TcpStream` type L96-171 — `= TcpStream` — ```
-  `TcpStream` type L173-177 — `= TcpStream` — ```
-  `from` function L174-176 — `(inner: StdTcpStream) -> Self` — ```
-  `TcpStream` type L179-183 — `impl Read for TcpStream` — ```
-  `read` function L180-182 — `(&mut self, buf: &mut [u8]) -> io::Result<usize>` — ```
-  `TcpStream` type L185-192 — `impl Write for TcpStream` — ```
-  `write` function L186-188 — `(&mut self, buf: &[u8]) -> io::Result<usize>` — ```
-  `flush` function L189-191 — `(&mut self) -> io::Result<()>` — ```
-  `read` function L198-200 — `(&mut self, buf: &mut [u8]) -> io::Result<usize>` — ```
-  `write` function L204-206 — `(&mut self, buf: &[u8]) -> io::Result<usize>` — ```
-  `flush` function L207-209 — `(&mut self) -> io::Result<()>` — ```
-  `tests` module L214-251 — `-` — ```
-  `connect_read_write_roundtrip` function L223-240 — `()` — The surface is portable: on the host it is a real socket.
-  `connect_refused_is_an_io_error` function L243-250 — `()` — ```
...
- pub `STATUS_INVALID_INDEX` variable L49 — `: i32` — The callee has no function at the requested dispatch index.
...
-  `Value` type L830-885 — `impl Serialize for Value` — records, options, and variants.
-  `serialize` function L831-884 — `(&self, serializer: S) -> Result<S::Ok, S::Error>` — records, options, and variants.
-  `Value` type L887-978 — `= Value` — records, options, and variants.
-  `deserialize` function L888-977 — `(deserializer: D) -> Result<Value, D::Error>` — records, options, and variants.
-  `ValueVisitor` struct L892 — `-` — records, options, and variants.
-  `ValueVisitor` type L893-975 — `= ValueVisitor` — records, options, and variants.
-  `Value` type L894 — `= Value` — records, options, and variants.
-  `expecting` function L895-897 — `(&self, f: &mut fmt::Formatter) -> fmt::Result` — records, options, and variants.
-  `visit_bool` function L898-900 — `(self, v: bool) -> Result<Value, E>` — records, options, and variants.
-  `visit_i64` function L901-903 — `(self, v: i64) -> Result<Value, E>` — records, options, and variants.
-  `visit_i128` function L904-911 — `(self, v: i128) -> Result<Value, E>` — records, options, and variants.
-  `visit_u64` function L912-914 — `(self, v: u64) -> Result<Value, E>` — records, options, and variants.
-  `visit_u128` function L915-922 — `(self, v: u128) -> Result<Value, E>` — records, options, and variants.
-  `visit_f64` function L923-925 — `(self, v: f64) -> Result<Value, E>` — records, options, and variants.
-  `visit_char` function L926-928 — `(self, v: char) -> Result<Value, E>` — records, options, and variants.
-  `visit_str` function L929-931 — `(self, v: &str) -> Result<Value, E>` — records, options, and variants.
-  `visit_string` function L932-934 — `(self, v: String) -> Result<Value, E>` — records, options, and variants.
-  `visit_bytes` function L935-937 — `(self, v: &[u8]) -> Result<Value, E>` — records, options, and variants.
-  `visit_byte_buf` function L938-940 — `(self, v: Vec<u8>) -> Result<Value, E>` — records, options, and variants.
-  `visit_unit` function L941-943 — `(self) -> Result<Value, E>` — records, options, and variants.
-  `visit_none` function L944-946 — `(self) -> Result<Value, E>` — records, options, and variants.
-  `visit_some` function L947-954 — `(self, deserializer: D) -> Result<Value, D::Error>` — records, options, and variants.
-  `visit_seq` function L955-964 — `(self, mut seq: A) -> Result<Value, A::Error>` — records, options, and variants.
-  `visit_map` function L965-974 — `(self, mut map: A) -> Result<Value, A::Error>` — records, options, and variants.
-  `tests` module L981-1114 — `-` — records, options, and variants.
-  `round_trip` function L985-992 — `(value: T)` — records, options, and variants.
-  `map_deserializes_from_a_list_of_pairs` function L995-1013 — `()` — records, options, and variants.
-  `Greeting` struct L1016-1020 — `{ name: String, times: u32, loud: bool }` — records, options, and variants.
-  `Wrapper` struct L1023 — `-` — records, options, and variants.
-  `Shape` enum L1026-1031 — `Unit | Newtype | Tuple | Struct` — records, options, and variants.
-  `primitives` function L1034-1044 — `()` — records, options, and variants.
-  `collections` function L1047-1053 — `()` — records, options, and variants.
-  `structs_and_maps` function L1056-1074 — `()` — records, options, and variants.
-  `enums` function L1077-1082 — `()` — records, options, and variants.
-  `nested` function L1085-1095 — `()` — records, options, and variants.
-  `Outer` struct L1087-1090 — `{ shapes: Vec<Shape>, tag: Option<String> }` — records, options, and variants.
-  `struct_shape_is_record` function L1098-1113 — `()` — records, options, and variants.
...
#### crates/fidius-guest/tests/proptest_wire.rs

-  `Concrete` enum L35-47 — `Bool | Int | Uint | Float | Text | Bytes | Unit | Opt | List | Map | Pair` — A representative nested payload — covers the shapes that cross the FFI boundary
-  `arb_concrete` function L49-73 — `() -> impl Strategy<Value = Concrete>` — it actually is: `from_value(to_value(&c)) == c`.
...
### crates/fidius-host/fuzz/fuzz_targets

> *Semantic summary to be generated by AI agent.*

#### crates/fidius-host/fuzz/fuzz_targets/wire_value.rs

-  `Payload` type L18 — `= Vec<(String, i64)>`
...
- pub `LoadError` enum L21-118 — `LibraryNotFound | SymbolNotFound | InvalidMagic | IncompatibleRegistryVersion | ...` — Errors that can occur when loading a plugin.
- pub `CallError` enum L122-183 — `Serialization | Deserialization | Plugin | Panic | BufferTooSmall | NotImplement...` — Errors that can occur when calling a plugin method.
-  `CallError` type L193-215 — `= CallError` — Fold the Python backend's call error into the unified [`CallError`].
-  `from` function L194-214 — `(e: fidius_python::PythonCallError) -> Self` — Error types for fidius-host plugin loading and calling.

#### crates/fidius-host/src/executor.rs

- pub `cdylib` module L44 — `-` — bincode `call_method`, keeping the bytes byte-identical to pre-refactor.
- pub `python` module L48 — `-` — bincode `call_method`, keeping the bytes byte-identical to pre-refactor.
- pub `wasm` module L50 — `-` — bincode `call_method`, keeping the bytes byte-identical to pre-refactor.
- pub `PluginExecutor` interface L70-81 — `{ fn info(), fn method_count(), fn call_raw() }` — The surface every execution backend shares.
- pub `ValueExecutor` interface L89-94 — `{ fn call() }` — Backends whose typed boundary is the self-describing [`Value`] model —
-  `body_tee` module L43 — `-` — `PluginExecutor` — the dispatch seam across execution backends.
-  `name_lookup` module L46 — `-` — bincode `call_method`, keeping the bytes byte-identical to pre-refactor.
...
- pub `configure_from_loaded` function L117-126 — `( plugin: crate::loader::LoadedPlugin, config: &C, ) -> Result<Self, LoadError>` — Construct a **configured** plugin instance from a DYNAMICALLY loaded
- pub `find_in_process_descriptor` function L130-134 — `( plugin_name: &str, ) -> Result<&'static PluginDescriptor, LoadError>` — Look up a descriptor in the current process's inventory registry by
- pub `from_python` function L140-144 — `(py: fidius_python::PythonPluginHandle, info: PluginInfo) -> Self` — Create a `PluginHandle` backed by a loaded Python plugin.
- pub `from_wasm` function L149-153 — `(executor: WasmComponentExecutor) -> Self` — Create a `PluginHandle` backed by a loaded WASM component.
- pub `call_method` function L160-188 — `( &self, index: usize, input: &I, ) -> Result<O, CallError>` — Call a plugin method by vtable index.
- pub `call_streaming` function L203-230 — `( &self, index: usize, input: &I, ) -> Result<crate::stream::ChunkStream, CallEr...` — Start a server-streaming method call by vtable index (FIDIUS-I-0026).
- pub `call_bidi_streaming` function L239-277 — `( &self, index: usize, items: impl IntoIterator<Item = I, IntoIter: Send + 'stat...` — Start a **bidirectional** streaming call (FIDIUS-I-0032 / ADR-0010): the host
- pub `call_method_raw` function L280-288 — `(&self, index: usize, input: &[u8]) -> Result<Vec<u8>, CallError>` — Call a `#[wire(raw)]` method: raw bytes in, raw bytes out, no bincode.
- pub `call_client_streaming_raw` function L300-321 — `( &self, index: usize, handle: *mut fidius_core::stream_ffi::FidiusStreamHandle,...` — Client-streaming raw call (FIDIUS-I-0030 CS2.2): pass the host's producer
- pub `call_client_streaming` function L329-374 — `( &self, method: usize, items: impl IntoIterator<Item = I, IntoIter: Send + 'sta...` — Typed client-streaming (FIDIUS-I-0030): the host produces `items` (the
- pub `bind_wasm_host_table` function L395-420 — `( &self, table: *const fidius_core::host_ffi::HostFunctionTable, ) -> Result<(),...` — Bind a host-function table (plugin → host callback channel) to a
- pub `has_capability` function L424-429 — `(&self, bit: u32) -> bool` — Check if an optional method is supported (capability bit set).
- pub `info` function L432-440 — `(&self) -> &PluginInfo` — Access the plugin's owned metadata.
- pub `method_metadata` function L445-454 — `(&self, method_id: u32) -> Vec<(&str, &str)>` — Static `#[method_meta(...)]` key/value metadata for the given method,
- pub `trait_metadata` function L458-466 — `(&self) -> Vec<(&str, &str)>` — Static `#[trait_meta(...)]` key/value metadata declared on the trait.
-  `Backend` enum L50-60 — `Cdylib | Python | Wasm` — The execution backend behind a [`PluginHandle`].
-  `PluginHandle` type L72-467 — `= PluginHandle` — refactor (`bincode(input)` straight to the FFI; `Value` is never involved).
-  `cdylib_stream_decode` function L475-481 — `( bytes: &[u8], ) -> Result<fidius_core::Value, CallError>` — Per-item decoder for the cdylib streaming fast path (FIDIUS-T-0137): each item
-  `lazy_bincode_producer` function L489-497 — `( items: impl IntoIterator<Item = I, IntoIter: Send + 'static>, ) -> Box<dyn Ite...` — A lazy, boxed bincode producer for the WASM client/bidi streaming input path: each
-  `lazy_json_producer` function L505-513 — `( items: impl IntoIterator<Item = I, IntoIter: Send + 'static>, ) -> Box<dyn Ite...` — A lazy, boxed producer of `Value`-shaped JSON for the Python client/bidi streaming
...
- pub `load_wasm_configured_with_grants` function L487-498 — `( &self, name: &str, descriptor: &'static fidius_core::wasm_descriptor::WasmInte...` — Load a **configured** WASM plugin with a caller-supplied capability
- pub `load_wasm_with_egress` function L506-513 — `( &self, name: &str, descriptor: &'static fidius_core::wasm_descriptor::WasmInte...` — Like [`Self::load_wasm`] but with a **per-plugin** `wasi:http` egress
...
-  `PluginHost` type L144-642 — `= PluginHost` — PluginHost builder and plugin discovery.
...
-  `load_wasm_impl` function L516-641 — `( &self, name: &str, descriptor: &'static fidius_core::wasm_descriptor::WasmInte...` — PluginHost builder and plugin discovery.
-  `is_dylib` function L645-654 — `(path: &Path) -> bool` — Check if a path has a platform-appropriate dylib extension.

#### crates/fidius-host/src/host_import.rs

- pub `HostImportInfo` struct L43-50 — `{ interface_name: String, interface_hash: u64, interface_version: u32 }` — Owned metadata about one host interface a plugin declares it can consume.
- pub `list_host_imports` function L104-125 — `(library: &Library) -> Result<Vec<HostImportInfo>, LoadError>` — List the host interfaces a loaded library declares it can consume.
- pub `bind_host_interface` function L146-213 — `( library: &Library, expected_name: &str, expected_hash: u64, expected_version: ...` — Install a host-function table into a loaded plugin library, gating on the
- pub `host_imports` function L218-220 — `(&self) -> Result<Vec<HostImportInfo>, LoadError>` — The host interfaces this library's plugins declare they can consume.
- pub `host_imports` function L226-228 — `(&self) -> Result<Vec<HostImportInfo>, LoadError>` — The host interfaces this plugin's library declares it can consume.
-  `GetHostImportsFn` type L53 — `= unsafe extern "C" fn() -> *const HostImportRegistry` — Signature of the optional `fidius_get_host_imports` export.
-  `read_registry` function L59-86 — `(library: &Library) -> Result<Option<&'static HostImportRegistry>, LoadError>` — Read and validate a library's host-import registry.
-  `descriptors` function L89-97 — `( registry: &'static HostImportRegistry, ) -> impl Iterator<Item = &'static Host...` — Iterate a validated registry's descriptors.
...
- pub `host` module L22 — `-`
- pub `host_import` module L23 — `-`
- pub `loader` module L24 — `-`
- pub `package` module L25 — `-`
- pub `signing` module L26 — `-`
- pub `stream` module L28 — `-`
- pub `types` module L29 — `-`
...
#### crates/fidius-host/src/executor/body_tee.rs

-  `REPLAY_CAP` variable L40 — `: usize` — Cap on the bytes a [`TeeBody`] captures for replay (64 KiB).
-  `CaptureState` enum L45-56 — `Incomplete | Complete | Overflowed | Trailers` — Terminal state of a tee capture, read at response time to decide whether
-  `CaptureInner` struct L58-61 — `{ buf: BytesMut, state: CaptureState }` — most common retryable requests).
-  `CaptureHandle` struct L65 — `-` — Shared handle onto a [`TeeBody`]'s capture, held by the dispatch task.
-  `CaptureHandle` type L67-81 — `= CaptureHandle` — most common retryable requests).
-  `state` function L71-73 — `(&self) -> CaptureState` — The capture's current terminal state (test observability; the dispatch
-  `replayable` function L77-80 — `(&self) -> Option<Bytes>` — The bytes to replay, if and only if the body was fully captured under
-  `TeeBody` struct L88-97 — `{ inner: HyperOutgoingBody, capture: Arc<Mutex<CaptureInner>>, stash: std::colle...` — A pass-through wrapper over a guest's outgoing body that captures its
-  `TeeBody` type L99-185 — `= TeeBody` — most common retryable requests).
-  `wrap` function L102-116 — `(inner: HyperOutgoingBody) -> (HyperOutgoingBody, CaptureHandle)` — Wrap `inner`, returning the teed body (boxed back to the
-  `prime` function L135-157 — `(&mut self)` — Drain whatever the inner body can yield *right now* (noop waker, no
-  `record_frame` function L159-176 — `(&self, frame: &Frame<Bytes>)` — most common retryable requests).
-  `record_end` function L178-184 — `(&self)` — most common retryable requests).
-  `TeeBody` type L187-231 — `impl Body for TeeBody` — most common retryable requests).
-  `Data` type L188 — `= Bytes` — most common retryable requests).
-  `Error` type L189 — `= ErrorCode` — most common retryable requests).
-  `poll_frame` function L191-210 — `( mut self: Pin<&mut Self>, cx: &mut Context<'_>, ) -> Poll<Option<Result<Frame<...` — most common retryable requests).
-  `is_end_stream` function L212-217 — `(&self) -> bool` — most common retryable requests).
-  `size_hint` function L219-230 — `(&self) -> SizeHint` — most common retryable requests).
-  `replay_body` function L235-239 — `(bytes: Bytes) -> HyperOutgoingBody` — A replayable body carrying previously captured bytes, for the retry
-  `tests` module L242-398 — `-` — most common retryable requests).
-  `FrameBody` struct L247 — `-` — Test body yielding a scripted sequence of frames.
-  `FrameBody` type L249-259 — `impl Body for FrameBody` — most common retryable requests).
-  `Data` type L250 — `= Bytes` — most common retryable requests).
-  `Error` type L251 — `= ErrorCode` — most common retryable requests).
-  `poll_frame` function L253-258 — `( mut self: Pin<&mut Self>, _cx: &mut Context<'_>, ) -> Poll<Option<Result<Frame...` — most common retryable requests).
-  `scripted` function L261-263 — `(frames: Vec<Frame<Bytes>>) -> HyperOutgoingBody` — most common retryable requests).
-  `drain` function L266-274 — `(mut body: HyperOutgoingBody) -> Vec<u8>` — Drain a body to end-of-stream, returning the concatenated data frames.
-  `PendingFirst` struct L279-282 — `{ pending_polls: usize, rest: std::collections::VecDeque<Frame<Bytes>> }` — A body wrapping only after its first poll would be Pending — the
-  `PendingFirst` type L284-298 — `impl Body for PendingFirst` — most common retryable requests).
-  `Data` type L285 — `= Bytes` — most common retryable requests).
-  `Error` type L286 — `= ErrorCode` — most common retryable requests).
-  `poll_frame` function L288-297 — `( mut self: Pin<&mut Self>, _cx: &mut Context<'_>, ) -> Poll<Option<Result<Frame...` — most common retryable requests).
-  `finished_body_is_complete_at_wrap_time` function L305-320 — `()` — Priming makes a body fully written before dispatch — the wasi-http
-  `small_body_passes_through_and_captures` function L323-335 — `()` — most common retryable requests).
-  `streaming_body_passes_through_and_completes_on_drain` function L338-352 — `()` — most common retryable requests).
-  `oversized_body_overflows_but_still_streams` function L355-366 — `()` — most common retryable requests).
-  `trailers_kill_the_capture` function L369-377 — `()` — most common retryable requests).
-  `undrained_streaming_body_stays_incomplete` function L380-391 — `()` — most common retryable requests).
-  `replay_body_round_trips` function L394-397 — `()` — most common retryable requests).
...
- pub `from_loaded` function L148-150 — `(plugin: crate::loader::LoadedPlugin) -> Self` — Create a CdylibExecutor from a LoadedPlugin.
- pub `from_loaded_with_config` function L159-173 — `(plugin: crate::loader::LoadedPlugin, cfg: &[u8]) -> Self` — Like [`Self::from_loaded`] but binds serialized `cfg` config bytes at
- pub `from_descriptor` function L182-184 — `(desc: &'static PluginDescriptor) -> Result<Self, LoadError>` — Create a CdylibExecutor from a plugin descriptor already registered in
- pub `from_descriptor_with_config` function L189-217 — `( desc: &'static PluginDescriptor, cfg: &[u8], ) -> Result<Self, LoadError>` — Like [`Self::from_descriptor`] but constructs the instance from serialized
- pub `find_in_process_descriptor` function L225-239 — `( plugin_name: &str, ) -> Result<&'static PluginDescriptor, LoadError>` — Look up a descriptor in the current process's inventory registry by
- pub `call_method` function L257-277 — `( &self, index: usize, input: &I, ) -> Result<O, CallError>` — Call a plugin method by vtable index.
- pub `call_method_raw` function L288-299 — `(&self, index: usize, input: &[u8]) -> Result<Vec<u8>, CallError>` — Call a plugin method whose argument and successful return value are
- pub `call_client_streaming_raw` function L575-659 — `( &self, index: usize, handle: *mut fidius_core::stream_ffi::FidiusStreamHandle,...` — Client-streaming raw call (FIDIUS-I-0030 CS2.2).
- pub `call_streaming_raw` function L758-812 — `( &self, index: usize, input_bytes: &[u8], decode_item: fn(&[u8]) -> Result<fidi...` — Start a server-streaming cdylib call (FIDIUS-I-0026 CS.1).
- pub `call_bidi_streaming_raw` function L825-888 — `( &self, index: usize, handle: *mut fidius_core::stream_ffi::FidiusStreamHandle,...` — Bidirectional streaming (FIDIUS-I-0032 / ADR-0010): call a method whose
- pub `has_capability` function L893-898 — `(&self, bit: u32) -> bool` — Check if an optional method is supported (capability bit is set).
- pub `info` function L901-903 — `(&self) -> &PluginInfo` — Access the plugin's owned metadata.
- pub `method_metadata` function L916-948 — `(&self, method_id: u32) -> Vec<(&str, &str)>` — Returns the static key/value metadata declared on the given method via
- pub `trait_metadata` function L954-975 — `(&self) -> Vec<(&str, &str)>` — Returns the static key/value metadata declared on the trait via
...
-  `CdylibExecutor` type L120-976 — `= CdylibExecutor` — (and future WASM) backends.
-  `new` function L123-145 — `( library: Arc<Library>, vtable: *const c_void, descriptor: *const PluginDescrip...` — Create a new CdylibExecutor.
-  `call_plugin_allocated` function L303-384 — `( &self, index: usize, input_bytes: &[u8], ) -> Result<O, CallError>` — PluginAllocated path: plugin allocates an output buffer via
-  `call_arena` function L390-477 — `( &self, index: usize, input_bytes: &[u8], ) -> Result<O, CallError>` — Arena path: host supplies a buffer from the thread-local pool.
-  `call_plugin_allocated_raw` function L482-563 — `( &self, index: usize, input_bytes: &[u8], ) -> Result<Vec<u8>, CallError>` — PluginAllocated raw path — same FFI shape as `call_plugin_allocated`,
-  `ClientStreamFn` type L587-594 — `= unsafe extern "C" fn( *mut c_void, *mut fidius_core::stream_ffi::FidiusStreamH...` — (and future WASM) backends.
-  `call_arena_raw` function L663-741 — `(&self, index: usize, input_bytes: &[u8]) -> Result<Vec<u8>, CallError>` — Arena raw path — same FFI shape as `call_arena`, success bytes
-  `ClientStreamFn` type L839-846 — `= unsafe extern "C" fn( *mut c_void, *mut fidius_core::stream_ffi::FidiusStreamH...` — (and future WASM) backends.
-  `CdylibExecutor` type L978-994 — `impl PluginExecutor for CdylibExecutor` — (and future WASM) backends.
-  `info` function L979-981 — `(&self) -> &PluginInfo` — (and future WASM) backends.
-  `method_count` function L983-985 — `(&self) -> u32` — (and future WASM) backends.
-  `call_raw` function L991-993 — `(&self, method: usize, input: &[u8]) -> Result<Vec<u8>, CallError>` — Raw byte dispatch.
-  `pump_stream_handle` function L1005-1106 — `( out_ptr: *mut u8, decode_item: fn(&[u8]) -> Result<fidius_core::Value, CallErr...` — Pump a returned `FidiusStreamHandle` into a [`crate::stream::ChunkStream`] on a
-  `STREAM_CHANNEL_CAP` variable L1015 — `: usize` — Bounded backpressure/memory window between the pump thread and the async
-  `SendHandle` struct L1019 — `-` — (and future WASM) backends.
-  `SendHandle` type L1020 — `impl Send for SendHandle` — (and future WASM) backends.
-  `INITIAL_ITEM_CAP` variable L1041 — `: usize` — (and future WASM) backends.

#### crates/fidius-host/src/executor/name_lookup.rs

-  `PinState` struct L63-66 — `{ by_name: HashMap<String, Vec<IpAddr>>, by_ip: HashMap<IpAddr, String> }` — What this store's lookups pinned: both directions of `name ↔ IPs`.
-  `PinState` type L68-92 — `= PinState` — upstream's implementation stands untouched.
-  `record` function L71-85 — `(&mut self, name: &str, ips: &[IpAddr])` — Record one completed resolution of `name` (already lowercased) to
-  `host_for` function L89-91 — `(&self, ip: &IpAddr) -> Option<String>` — The name this store's lookups most recently resolved to `ip`
-  `PinTable` type L99 — `= Arc<Mutex<PinState>>` — Shared handle to a store's pins: one clone lives in `HostState` (feeding
-  `Resolver` type L105 — `= Arc<dyn Fn(&str) -> std::io::Result<Vec<IpAddr>> + Send + Sync>` — The host-side resolution function.
-  `default_resolver` function L107-109 — `() -> Resolver` — upstream's implementation stands untouched.
-  `FidiusNameLookup` struct L112 — `-` — `HasData` marker for the shadowed instance (the `D` in `add_to_linker`).
-  `FidiusNameLookup` type L114-116 — `impl HasData for FidiusNameLookup` — upstream's implementation stands untouched.
-  `Data` type L115 — `= NameLookupView<'a>` — upstream's implementation stands untouched.
-  `NameLookupView` struct L122-127 — `{ table: &'a mut ResourceTable, pins: &'a PinTable, policy: Option<&'a Arc<dyn E...` — Per-call view the bindgen host traits run against: the store's own
-  `convert_error_code` function L134-136 — `(&mut self, error: SocketError) -> wasmtime::Result<ErrorCode>` — upstream's implementation stands untouched.
-  `network_error_code` function L138-147 — `( &mut self, err: Resource<wasmtime::Error>, ) -> wasmtime::Result<Option<ErrorC...` — upstream's implementation stands untouched.
-  `drop` function L151-154 — `(&mut self, this: Resource<Network>) -> wasmtime::Result<()>` — upstream's implementation stands untouched.
-  `resolve_addresses` function L158-209 — `( &mut self, network: Resource<Network>, name: String, ) -> Result<Resource<Reso...` — upstream's implementation stands untouched.
-  `resolve_next_address` function L215-235 — `( &mut self, resource: Resource<ResolveAddressStream>, ) -> Result<Option<IpAddr...` — upstream's implementation stands untouched.
-  `subscribe` function L237-242 — `( &mut self, resource: Resource<ResolveAddressStream>, ) -> wasmtime::Result<Res...` — upstream's implementation stands untouched.
-  `drop` function L244-247 — `(&mut self, resource: Resource<ResolveAddressStream>) -> wasmtime::Result<()>` — upstream's implementation stands untouched.
-  `tests` module L251-314 — `-` — upstream's implementation stands untouched.
-  `ip` function L254-256 — `(s: &str) -> IpAddr` — upstream's implementation stands untouched.
-  `record_and_lookup` function L259-271 — `()` — upstream's implementation stands untouched.
-  `re_resolve_replaces_wholesale` function L274-284 — `()` — upstream's implementation stands untouched.
-  `ip_collision_most_recent_wins_without_clobbering_on_stale_removal` function L287-306 — `()` — upstream's implementation stands untouched.
-  `default_resolver_resolves_localhost` function L309-313 — `()` — upstream's implementation stands untouched.
...
- pub `call_client_streaming` function L54-67 — `( &self, method: usize, items: Box<dyn Iterator<Item = serde_json::Value> + Send...` — Client-streaming (FIDIUS-I-0030 CS2.4): the host produces `items`; the plugin
- pub `call_bidi_streaming` function L134-147 — `( &self, method: usize, items: Box<dyn Iterator<Item = serde_json::Value> + Send...` — Bidirectional streaming (FIDIUS-I-0032 / ADR-0010): the host produces `items`
-  `Pyo3Executor` type L44-68 — `= Pyo3Executor` — routed through the neutral `Value` currency.
-  `Pyo3Executor` type L70-83 — `impl PluginExecutor for Pyo3Executor` — routed through the neutral `Value` currency.
-  `info` function L71-73 — `(&self) -> &PluginInfo` — routed through the neutral `Value` currency.
-  `method_count` function L75-77 — `(&self) -> u32` — routed through the neutral `Value` currency.
-  `call_raw` function L79-82 — `(&self, method: usize, input: &[u8]) -> Result<Vec<u8>, CallError>` — routed through the neutral `Value` currency.
-  `Pyo3Executor` type L85-98 — `impl ValueExecutor for Pyo3Executor` — routed through the neutral `Value` currency.
-  `call` function L86-97 — `(&self, method: usize, args: Value) -> Result<Value, CallError>` — routed through the neutral `Value` currency.
-  `STREAM_CHANNEL_CAP` variable L105 — `: usize` — Bounded channel depth between the GIL-holding pump thread and the host's
-  `Pyo3Executor` type L109-126 — `= Pyo3Executor` — routed through the neutral `Value` currency.
-  `call_streaming` function L110-125 — `( &self, method: usize, args: Value, ) -> Result<crate::stream::ChunkStream, Cal...` — routed through the neutral `Value` currency.
-  `Pyo3Executor` type L129-148 — `= Pyo3Executor` — routed through the neutral `Value` currency.
-  `pump_python_stream` function L155-194 — `(stream: fidius_python::PythonStream) -> crate::stream::ChunkStream` — Pump a `PythonStream` (a guest generator) into a [`crate::stream::ChunkStream`] on a

#### crates/fidius-host/src/executor/wasm.rs

- pub `EgressDenied` struct L64-68 — `{ reason: String }` — Denial returned by an [`EgressPolicy`] to refuse an outbound request.
- pub `new` function L72-76 — `(reason: impl Into<String>) -> Self` — A denial with a reason.
- pub `ResponseDirective` enum L101-118 — `Forward | RetryOnce` — Embedder-supplied policy governing a sandboxed WASM guest's **outbound HTTP**
- pub `TcpTarget` struct L120-130 — `{ host: Option<&'a str>, addr: SocketAddr }` — from the package manifest's allow-list.
- pub `EgressPolicy` interface L132-259 — `{ fn authorize(), fn authorize_tcp(), fn authorize_tcp_target(), fn authorize_dn...` — from the package manifest's allow-list.
- pub `WasmMethod` struct L848-856 — `{ name: String, wire_raw: bool, streaming: bool }` — A method on the WASM interface, in declaration (vtable) order.
- pub `WasmComponentExecutor` struct L859-896 — `{ engine: Engine, instance_pre: InstancePre<HostState>, interface: String, metho...` — WASM component execution backend.
- pub `from_component_bytes` function L907-915 — `( bytes: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Build an executor from raw component bytes (a `.wasm` component).
- pub `from_component_bytes_with_egress` function L920-943 — `( bytes: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Like [`Self::from_component_bytes`] but with an embedder [`EgressPolicy`]
- pub `from_cwasm` function L951-959 — `( cwasm: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Build from a precompiled `.cwasm` (engine/version-specific).
- pub `from_cwasm_with_egress` function L967-990 — `( cwasm: &[u8], interface: String, methods: Vec<WasmMethod>, capabilities: Vec<S...` — Like [`Self::from_cwasm`] but with an embedder [`EgressPolicy`]
- pub `set_resolver` function L1174-1179 — `( &mut self, resolver: Arc<dyn Fn(&str) -> std::io::Result<Vec<IpAddr>> + Send +...` — Replace the host-side resolver behind the shadowed `ip-name-lookup`
- pub `bind_host_table` function L1193-1225 — `( &self, table: *const fidius_core::host_ffi::HostFunctionTable, ) -> Result<(),...` — Bind a host-function table (plugin → host callback channel) to this
- pub `configure` function L1231-1249 — `(&mut self, cfg: &[u8]) -> Result<(), CallError>` — Bind config once (FIDIUS-A-0006 / CI.3): instantiate a *persistent* store,
- pub `call_client_streaming` function L1256-1304 — `( &self, method: usize, producer: Box<dyn Iterator<Item = Vec<u8>> + Send>, args...` — Client-streaming (FIDIUS-I-0030 CS2.3): call a method whose `Stream<T>`
- pub `call_bidi_streaming` function L1312-1320 — `( &self, method: usize, producer: Box<dyn Iterator<Item = Vec<u8>> + Send>, args...` — Bidirectional streaming (FIDIUS-I-0032 / ADR-0010): the host produces `producer`
- pub `interface_hash` function L1424-1440 — `(&self) -> Result<u64, CallError>` — Call the `fidius-interface-hash` export — the integrity check the loader
- pub `validate_component` function L1958-1966 — `(bytes: &[u8]) -> Result<(), CallError>` — Validate that `bytes` is a well-formed WASM **component** (Component Model),
- pub `precompile_component` function L1972-1980 — `(bytes: &[u8]) -> Result<Vec<u8>, CallError>` — Ahead-of-time compile a component into engine/version-specific `.cwasm`
-  `EgressDenied` type L70-77 — `= EgressDenied` — from the package manifest's allow-list.
-  `authorize_tcp` function L156-158 — `(&self, _addr: &SocketAddr) -> Result<(), EgressDenied>` — Authorize one outbound **TCP** connection before `connect` (FIDIUS-I-0033).
-  `authorize_tcp_target` function L175-177 — `(&self, target: &TcpTarget<'_>) -> Result<(), EgressDenied>` — Name-aware TCP authorization (FIDIUS-I-0034): like
-  `authorize_dns` function L191-193 — `(&self, _name: &str) -> Result<(), EgressDenied>` — Authorize one guest DNS lookup, **before** resolution (FIDIUS-I-0034).
-  `authorize_udp` function L210-212 — `(&self, _addr: &SocketAddr) -> Result<(), EgressDenied>` — Authorize one outbound **UDP** datagram before it leaves (FIDIUS-I-0033) —
-  `observes_responses` function L221-223 — `(&self) -> bool` — Opt-in gate for [`on_response`](Self::on_response) (FIDIUS-I-0035).
-  `on_response` function L249-258 — `( &self, request: &http::request::Parts, status: http::StatusCode, headers: &htt...` — Observe the response HEAD for a request this policy authorized,
-  `EgressHooks` struct L266-268 — `{ policy: Option<Arc<dyn EgressPolicy>> }` — fidius's [`WasiHttpHooks`] adapter: routes every outbound request through the
-  `EgressHooks` type L270-312 — `impl WasiHttpHooks for EgressHooks` — from the package manifest's allow-list.
-  `send_request` function L271-311 — `( &mut self, request: http::Request<HyperOutgoingBody>, config: OutgoingRequestC...` — from the package manifest's allow-list.
-  `copy_config` function L317-324 — `(c: &OutgoingRequestConfig) -> OutgoingRequestConfig` — Copy an [`OutgoingRequestConfig`] (all-`Copy` fields; the type itself isn't
-  `dispatch_observed` function L332-381 — `( policy: Arc<dyn EgressPolicy>, original: http::request::Parts, dispatched: htt...` — The observing dispatch (FIDIUS-I-0035): send the request, show the policy
-  `HostState` struct L386-408 — `{ ctx: WasiCtx, table: ResourceTable, http_ctx: WasiHttpCtx, hooks: EgressHooks,...` — Per-store host state.
-  `name_lookup_view` function L413-420 — `(state: &mut HostState) -> NameLookupView<'_>` — Accessor handed to `ip_name_lookup::add_to_linker` for the shadowed
-  `HostTables` type L424 — `= Arc<std::sync::RwLock<std::collections::HashMap<String, HostTableRef>>>` — The executor-wide registry of bound host-function tables, keyed by
-  `HostTableRef` struct L430 — `-` — A `Send + Sync` wrapper for a bound, process-lifetime
-  `HostTableRef` type L435 — `impl Send for HostTableRef` — from the package manifest's allow-list.
-  `HostTableRef` type L436 — `impl Sync for HostTableRef` — from the package manifest's allow-list.
-  `dispatch_host_table` function L441-472 — `( table: &fidius_core::host_ffi::HostFunctionTable, index: u32, args: &[u8], ) -...` — Run one host-function dispatch through a bound table and return the raw
-  `HostState` type L474-482 — `impl WasiHttpView for HostState` — from the package manifest's allow-list.
-  `http` function L475-481 — `(&mut self) -> WasiHttpCtxView<'_>` — from the package manifest's allow-list.
-  `KNOWN_CAPABILITIES` variable L490-510 — `: &[&str]` — Capabilities the host knows how to grant.
-  `validate_capabilities` function L514-596 — `(caps: &[String]) -> Result<(), CallError>` — Reject unknown capability names early (at load) so a typo fails closed and
-  `build_wasi_ctx` function L606-755 — `( caps: &[String], egress: Option<Arc<dyn EgressPolicy>>, pins: PinTable, ) -> W...` — Build a `WasiCtx` from the allow-list.
-  `is_blocked_ip` function L762-781 — `(ip: &IpAddr) -> bool` — Baseline SSRF denylist for the raw-socket grant (FIDIUS-T-0143): an address a
-  `HOST_WASI_HTTP` variable L791 — `: (u32, u32, u32)` — The `wasi:http` version this host provides — what `wasmtime-wasi-http`
-  `wasi_http_incompatibility` function L803-833 — `(import_names: impl Iterator<Item = &'a str>) -> Option<String>` — Scan a component's import names for a `wasi:http` version this host can't
-  `HostState` type L837-844 — `impl WasiView for HostState` — from the package manifest's allow-list.
-  `ctx` function L838-843 — `(&mut self) -> WasiCtxView<'_>` — from the package manifest's allow-list.
-  `ConfiguredStore` struct L899-902 — `{ store: Store<HostState>, instance: wasmtime::component::Instance }` — A configured instance's persistent store + instance (FIDIUS-A-0006 / CI.3).
-  `WasmComponentExecutor` type L904-1441 — `= WasmComponentExecutor` — from the package manifest's allow-list.
-  `build` function L994-1166 — `( engine: Engine, component: &Component, interface: String, methods: Vec<WasmMet...` — Shared constructor: wire WASI into a `Linker` and pre-instantiate the
-  `with_store` function L1324-1339 — `( &self, f: impl FnOnce(&mut Store<HostState>, &wasmtime::component::Instance) -...` — Run `f` with a `(store, instance)`: the persistent configured store if
-  `instantiate` function L1344-1372 — `(&self) -> Result<(Store<HostState>, wasmtime::component::Instance), CallError>` — Instantiate a fresh sandboxed `Store` + component instance from the cached
-  `func` function L1375-1402 — `( &self, store: &mut Store<HostState>, instance: &wasmtime::component::Instance,...` — Resolve an exported function within the plugin's interface by name.
-  `method` function L1404-1420 — `(&self, index: usize, want_raw: bool) -> Result<&WasmMethod, CallError>` — from the package manifest's allow-list.
-  `WasmComponentExecutor` type L1443-1478 — `impl PluginExecutor for WasmComponentExecutor` — from the package manifest's allow-list.
-  `info` function L1444-1446 — `(&self) -> &PluginInfo` — from the package manifest's allow-list.
-  `method_count` function L1448-1450 — `(&self) -> u32` — from the package manifest's allow-list.
-  `call_raw` function L1452-1477 — `(&self, method: usize, input: &[u8]) -> Result<Vec<u8>, CallError>` — from the package manifest's allow-list.
-  `WasmComponentExecutor` type L1480-1525 — `impl ValueExecutor for WasmComponentExecutor` — from the package manifest's allow-list.
-  `call` function L1481-1524 — `(&self, method: usize, args: Value) -> Result<Value, CallError>` — from the package manifest's allow-list.
-  `STREAM_CHANNEL_CAP` variable L1531 — `: usize` — Bounded channel depth between the wasmtime pump thread and the async
-  `WasmComponentExecutor` type L1535-1543 — `= WasmComponentExecutor` — from the package manifest's allow-list.
-  `call_streaming` function L1536-1542 — `( &self, method: usize, args: Value, ) -> Result<crate::stream::ChunkStream, Cal...` — from the package manifest's allow-list.
-  `WasmComponentExecutor` type L1545-1678 — `= WasmComponentExecutor` — from the package manifest's allow-list.
-  `stream_with_producer` function L1552-1677 — `( &self, method: usize, args: Value, producer: Option<Box<dyn Iterator<Item = Ve...` — Shared server-streaming / bidirectional output pump.
-  `plugin_error_from_val` function L1682-1708 — `(payload: Option<&Val>) -> CallError` — Map a `result::err` payload (expected: a record with `code`/`message`/
-  `to_kebab` function L1713-1728 — `(s: &str) -> String` — fidius `Value` → wasmtime `Val`.
-  `kebab_to_snake` function L1731-1733 — `(s: &str) -> String` — kebab-case → snake_case (WIT record field → serde struct field).
-  `kebab_to_pascal` function L1736-1746 — `(s: &str) -> String` — kebab-case → PascalCase (WIT variant case → serde enum variant).
-  `value_to_val` function L1748-1796 — `(v: &Value) -> Result<Val, CallError>` — from the package manifest's allow-list.
-  `value_to_val_typed` function L1803-1907 — `(v: &Value, ty: &wasmtime::component::Type) -> Result<Val, CallError>` — Type-directed lowering for the **argument** path.
-  `val_to_value` function L1910-1948 — `(v: &Val) -> Value` — wasmtime `Val` → fidius `Value` (structural; self-describing).
-  `ssrf_tests` module L1983-2021 — `-` — from the package manifest's allow-list.
-  `ip` function L1987-1989 — `(s: &str) -> IpAddr` — from the package manifest's allow-list.
-  `blocks_internal_and_metadata_targets` function L1992-2008 — `()` — from the package manifest's allow-list.
-  `allows_public_targets` function L2011-2020 — `()` — from the package manifest's allow-list.
-  `fs_capability_tests` module L2024-2063 — `-` — from the package manifest's allow-list.
-  `msg` function L2027-2032 — `(r: Result<(), CallError>) -> String` — from the package manifest's allow-list.
-  `path_scoped_fs_grants_are_accepted` function L2035-2040 — `()` — from the package manifest's allow-list.
-  `bare_filesystem_is_rejected` function L2043-2047 — `()` — from the package manifest's allow-list.
-  `fs_grant_without_a_path_is_rejected` function L2050-2053 — `()` — from the package manifest's allow-list.
-  `build_wasi_ctx_with_an_fs_grant_does_not_panic` function L2056-2062 — `()` — from the package manifest's allow-list.
-  `tcp_egress_tests` module L2066-2174 — `-` — from the package manifest's allow-list.
-  `AllowOnePort` struct L2072 — `-` — A reference embedder policy: allow TCP to one allow-listed `host:port`
-  `AllowOnePort` type L2073-2084 — `impl EgressPolicy for AllowOnePort` — from the package manifest's allow-list.
-  `authorize` function L2074-2076 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `authorize_tcp` function L2077-2083 — `(&self, addr: &SocketAddr) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `tcp_is_a_known_capability` function L2087-2090 — `()` — from the package manifest's allow-list.
-  `udp_is_a_known_capability` function L2093-2097 — `()` — from the package manifest's allow-list.
-  `policy_egress_and_network_are_mutually_exclusive` function L2100-2126 — `()` — from the package manifest's allow-list.
-  `default_authorize_tcp_and_udp_deny` function L2129-2141 — `()` — from the package manifest's allow-list.
-  `HttpOnly` struct L2132 — `-` — from the package manifest's allow-list.
-  `HttpOnly` type L2133-2137 — `impl EgressPolicy for HttpOnly` — from the package manifest's allow-list.
-  `authorize` function L2134-2136 — `(&self, _p: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `tcp_grant_with_policy_builds_a_ctx` function L2144-2151 — `()` — from the package manifest's allow-list.
-  `tcp_grant_without_policy_builds_a_ctx` function L2154-2158 — `()` — from the package manifest's allow-list.
-  `udp_and_combined_grants_build_a_ctx` function L2161-2173 — `()` — from the package manifest's allow-list.
-  `wasi_http_version_tests` module L2177-2221 — `-` — from the package manifest's allow-list.
-  `host_matched_version_is_compatible` function L2181-2196 — `()` — from the package manifest's allow-list.
-  `newer_minor_or_patch_is_rejected_with_a_clear_message` function L2199-2211 — `()` — from the package manifest's allow-list.
-  `no_wasi_http_import_is_fine` function L2214-2220 — `()` — from the package manifest's allow-list.
-  `egress_policy_tests` module L2224-2308 — `-` — from the package manifest's allow-list.
-  `LegacyLoopbackOnly` struct L2229 — `-` — A policy written before FIDIUS-I-0034: overrides ONLY `authorize_tcp`.
-  `LegacyLoopbackOnly` type L2230-2241 — `impl EgressPolicy for LegacyLoopbackOnly` — from the package manifest's allow-list.
-  `authorize` function L2231-2233 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `authorize_tcp` function L2234-2240 — `(&self, addr: &SocketAddr) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `authorize_tcp_target_default_delegates_to_authorize_tcp` function L2244-2270 — `()` — from the package manifest's allow-list.
-  `authorize_dns_defaults_to_allow` function L2273-2279 — `()` — from the package manifest's allow-list.
-  `response_hook_defaults_are_opt_out` function L2283-2307 — `()` — FIDIUS-I-0035: the response hook is strictly opt-in.
-  `response_hook_dispatch_tests` module L2314-2639 — `-` — FIDIUS-I-0035: the observing dispatch (`dispatch_observed`) against real
-  `R401` variable L2325 — `: &str` — from the package manifest's allow-list.
-  `R200` variable L2326 — `: &str` — from the package manifest's allow-list.
-  `scripted_server` function L2331-2356 — `( responses: &'static [&'static str], ) -> (String, std::sync::mpsc::Receiver<St...` — Scripted loopback server: serves `responses` in order, one connection
-  `parts_for` function L2358-2366 — `(addr: &str) -> http::request::Parts` — from the package manifest's allow-list.
-  `config` function L2368-2376 — `() -> OutgoingRequestConfig` — from the package manifest's allow-list.
-  `ChannelShapedEmpty` struct L2381 — `-` — An empty body in the shape wasi-http's `BodyImpl` hands us: end only
-  `ChannelShapedEmpty` type L2382-2391 — `= ChannelShapedEmpty` — from the package manifest's allow-list.
-  `Data` type L2383 — `= bytes::Bytes` — from the package manifest's allow-list.
-  `Error` type L2384 — `= super::ErrorCode` — from the package manifest's allow-list.
-  `poll_frame` function L2385-2390 — `( self: std::pin::Pin<&mut Self>, _cx: &mut std::task::Context<'_>, ) -> std::ta...` — from the package manifest's allow-list.
-  `run` function L2395-2405 — `( policy: Arc<dyn EgressPolicy>, addr: &str, ) -> Result<super::IncomingResponse...` — Run the observing dispatch the way `send_request` does: pre-authorize
-  `RefreshOn401` struct L2410-2413 — `{ stamps: AtomicUsize, observations: Mutex<Vec<(u16, bool)>> }` — The motivating policy: stamps a credential, refreshes it on 401.
-  `RefreshOn401` type L2415-2422 — `= RefreshOn401` — from the package manifest's allow-list.
-  `new` function L2416-2421 — `() -> Self` — from the package manifest's allow-list.
-  `RefreshOn401` type L2424-2455 — `impl EgressPolicy for RefreshOn401` — from the package manifest's allow-list.
-  `authorize` function L2425-2434 — `(&self, parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `observes_responses` function L2435-2437 — `(&self) -> bool` — from the package manifest's allow-list.
-  `on_response` function L2438-2454 — `( &self, _request: &http::request::Parts, status: http::StatusCode, _headers: &h...` — from the package manifest's allow-list.
-  `retry_once_restamps_and_forwards_the_second_response` function L2458-2477 — `()` — from the package manifest's allow-list.
-  `DenyOnRetry` struct L2481 — `-` — A policy that consumes the 401 and then refuses to re-stamp: the guest
-  `DenyOnRetry` type L2483-2502 — `impl EgressPolicy for DenyOnRetry` — from the package manifest's allow-list.
-  `authorize` function L2484-2489 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `observes_responses` function L2490-2492 — `(&self) -> bool` — from the package manifest's allow-list.
-  `on_response` function L2493-2501 — `( &self, _request: &http::request::Parts, _status: http::StatusCode, _headers: &...` — from the package manifest's allow-list.
-  `deny_on_retry_maps_to_request_denied` function L2505-2516 — `()` — from the package manifest's allow-list.
-  `AlwaysRetry` struct L2521 — `-` — An always-RetryOnce policy is bounded to a single retry: the second
-  `AlwaysRetry` type L2523-2539 — `impl EgressPolicy for AlwaysRetry` — from the package manifest's allow-list.
-  `authorize` function L2524-2526 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — from the package manifest's allow-list.
-  `observes_responses` function L2527-2529 — `(&self) -> bool` — from the package manifest's allow-list.
-  `on_response` function L2530-2538 — `( &self, _request: &http::request::Parts, _status: http::StatusCode, _headers: &...` — from the package manifest's allow-list.
-  `retry_is_bounded_to_one` function L2542-2549 — `()` — from the package manifest's allow-list.
-  `oversized_body_forwards` function L2554-2591 — `()` — A body streaming past the 64 KiB cap is not replayable: the 401
-  `non_replayable_body_forwards` function L2596-2638 — `()` — A non-replayable body (here: carrying trailers) forwards the 401
-  `TrailerBody` struct L2601 — `-` — from the package manifest's allow-list.
-  `TrailerBody` type L2602-2615 — `impl Body for TrailerBody` — from the package manifest's allow-list.
-  `Data` type L2603 — `= bytes::Bytes` — from the package manifest's allow-list.
-  `Error` type L2604 — `= super::ErrorCode` — from the package manifest's allow-list.
-  `poll_frame` function L2605-2614 — `( mut self: Pin<&mut Self>, _cx: &mut Context<'_>, ) -> Poll<Option<Result<Frame...` — from the package manifest's allow-list.

### crates/fidius-host/tests

> *Semantic summary to be generated by AI agent.*

#### crates/fidius-host/tests/cdylib_bidi_stream_e2e.rs

- pub `Transformer` interface L30-33 — `{ fn transform() }` — `call_bidi_streaming` + drop-cancel.
- pub `Doubler` struct L35 — `-` — `call_bidi_streaming` + drop-cancel.
-  `Doubler` type L38-45 — `impl Transformer for Doubler` — `call_bidi_streaming` + drop-cancel.
...
#### crates/fidius-host/tests/configured_cdylib_dynamic_e2e.rs

-  `GreetConfig` struct L30-32 — `{ greeting: String }` — configured provider cdylib at runtime and bind its config once.
-  `smoke_host` function L35-44 — `() -> PluginHost` — Build the smoke dylib and return a host whose search path finds it.
-  `dynamic_cdylib_binds_config_at_load` function L47-63 — `()` — configured provider cdylib at runtime and bind its config once.
-  `n_dynamically_configured_instances_coexist` function L66-87 — `()` — configured provider cdylib at runtime and bind its config once.
...
-  `plugin_source_dir` function L40-42 — `() -> PathBuf` — therefore a separate process with its own dylib load.
-  `RecordingHost` struct L47-52 — `{ calls: Mutex<Vec<(String, std::thread::ThreadId)>>, reclaims: AtomicU32 }` — Recording host implementation.
-  `RecordingHost` type L54-66 — `= RecordingHost` — therefore a separate process with its own dylib load.
-  `record` function L55-62 — `(&self, entry: impl Into<String>)` — therefore a separate process with its own dylib load.
-  `calls` function L63-65 — `(&self) -> Vec<(String, std::thread::ThreadId)>` — therefore a separate process with its own dylib load.
-  `RecordingHost` type L68-95 — `impl TestHost for RecordingHost` — therefore a separate process with its own dylib load.
-  `release_slot` function L69-80 — `(&self, task_id: String) -> Result<(), PluginError>` — therefore a separate process with its own dylib load.
-  `reclaim_slot` function L82-85 — `(&self, task_id: String) -> Result<u32, PluginError>` — therefore a separate process with its own dylib load.
-  `get_value` function L87-90 — `(&self, key: String) -> String` — therefore a separate process with its own dylib load.
-  `panicky` function L92-94 — `(&self) -> u32` — therefore a separate process with its own dylib load.
-  `bound_plugin` function L98-127 — `() -> &'static (PluginHandle, Arc<RecordingHost>)` — One shared loaded-plugin + bound-host state for the whole test binary.
-  `STATE` variable L99 — `: OnceLock<(PluginHandle, Arc<RecordingHost>)>` — therefore a separate process with its own dylib load.
-  `reentrant_host_plugin_host_roundtrip` function L130-157 — `()` — therefore a separate process with its own dylib load.
-  `host_error_arrives_typed_in_the_plugin` function L160-178 — `()` — therefore a separate process with its own dylib load.
-  `host_panic_surfaces_as_typed_error_not_unwinding` function L181-190 — `()` — therefore a separate process with its own dylib load.
-  `plugin_panic_after_callback_surfaces_as_call_error` function L193-202 — `()` — therefore a separate process with its own dylib load.
-  `callbacks_work_from_plugin_spawned_threads` function L205-211 — `()` — therefore a separate process with its own dylib load.
-  `plugin_reports_interface_bound` function L214-220 — `()` — therefore a separate process with its own dylib load.
-  `second_bind_fails_loudly_without_disturbing_the_first` function L223-244 — `()` — therefore a separate process with its own dylib load.
-  `version_mismatch_fails_at_bind_and_never_builds_a_table` function L247-275 — `()` — therefore a separate process with its own dylib load.
-  `signature_drift_fails_at_bind_and_never_builds_a_table` function L278-299 — `()` — therefore a separate process with its own dylib load.
-  `binding_an_undeclared_interface_is_a_clean_no` function L302-316 — `()` — therefore a separate process with its own dylib load.

#### crates/fidius-host/tests/host_functions_in_process.rs

-  `CountingHost` struct L33-35 — `{ reclaims: AtomicU32 }` — host-function test binaries.
-  `CountingHost` type L37-50 — `impl TestHost for CountingHost` — host-function test binaries.
-  `release_slot` function L38-40 — `(&self, _task_id: String) -> Result<(), PluginError>` — host-function test binaries.
-  `reclaim_slot` function L41-43 — `(&self, _task_id: String) -> Result<u32, PluginError>` — host-function test binaries.
-  `get_value` function L44-46 — `(&self, key: String) -> String` — host-function test binaries.
-  `panicky` function L47-49 — `(&self) -> u32` — host-function test binaries.
-  `doctored_tables_are_rejected_by_the_plugin_shim` function L56-89 — `()` — A doctored table must be rejected by the plugin-side bind shim before
-  `in_process_bind_and_reentrant_call` function L92-114 — `()` — host-function test binaries.

#### crates/fidius-host/tests/host_functions_unbound.rs

-  `hostcall_plugin_dir` function L29-31 — `() -> PathBuf` — process in which `TestHost` was **never** bound.
-  `smoke_plugin_dir` function L33-35 — `() -> PathBuf` — process in which `TestHost` was **never** bound.
-  `unbound_host_interface_is_a_typed_error_not_a_crash` function L38-61 — `()` — process in which `TestHost` was **never** bound.
-  `plugin_without_host_interfaces_loads_and_runs_unchanged` function L64-95 — `()` — process in which `TestHost` was **never** bound.

#### crates/fidius-host/tests/hostname_egress_e2e.rs

-  `IFACE` variable L37 — `: &str` — IP literals.
-  `tcp_echo_component` function L41-60 — `() -> Option<Vec<u8>>` — IP literals.
-  `BYTES` variable L42 — `: OnceLock<Option<Vec<u8>>>` — IP literals.
-  `mock_tcp_echo_once` function L62-64 — `(n: usize) -> (u16, std::thread::JoinHandle<()>)` — IP literals.
-  `mock_tcp_echo_once_on` function L69-82 — `(host: &str, n: usize) -> (u16, std::thread::JoinHandle<()>)` — Like `mock_tcp_echo_once`, but bound to a chosen loopback flavor — the
-  `NameAllowList` struct L88-92 — `{ allowed: Vec<String>, seen: Arc<Mutex<Vec<(Option<String>, SocketAddr)>>>, den...` — What the check handed the policy, recorded for assertions: the future
-  `NameAllowList` type L94-102 — `= NameAllowList` — IP literals.
-  `new` function L95-101 — `(allowed: &[&str]) -> Self` — IP literals.
-  `NameAllowList` type L104-127 — `impl EgressPolicy for NameAllowList` — IP literals.
-  `authorize` function L105-107 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — IP literals.
-  `authorize_tcp_target` function L111-120 — `(&self, target: &TcpTarget<'_>) -> Result<(), EgressDenied>` — IP literals.
-  `authorize_dns` function L121-126 — `(&self, name: &str) -> Result<(), EgressDenied>` — IP literals.
-  `TestResolver` type L131 — `= Arc<dyn Fn(&str) -> std::io::Result<Vec<IpAddr>> + Send + Sync>` — The executor's `#[doc(hidden)]` resolver seam — tests inject one to model
-  `CONNECT_AND_ECHO` variable L134 — `: usize` — Method indices, in the order `load_with` declares them.
-  `CONNECT_SEQ` variable L135 — `: usize` — IP literals.
-  `load` function L137-139 — `(policy: Arc<dyn EgressPolicy>) -> PluginHandle` — IP literals.
-  `load_with` function L141-177 — `(policy: Arc<dyn EgressPolicy>, resolver: Option<TestResolver>) -> PluginHandle` — IP literals.
-  `hostname_dial_reaches_policy_with_name_and_echoes` function L186-208 — `()` — Production path, initiative acceptance criterion 1 (hostname half): a guest
-  `ip_literal_dial_reaches_policy_as_none_and_is_denied` function L214-234 — `()` — Production path, criterion 1 (IP half) + pin correctness: an IP-literal
-  `authorize_dns_denial_fails_lookup_before_connect` function L240-258 — `()` — `authorize_dns` denial: the guest's lookup fails before resolution — the
-  `LegacyLoopbackOnly` struct L264 — `-` — Default delegation, e2e flavor (initiative criterion 2): a pre-0034 policy
-  `LegacyLoopbackOnly` type L265-276 — `impl EgressPolicy for LegacyLoopbackOnly` — IP literals.
-  `authorize` function L266-268 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — IP literals.
-  `authorize_tcp` function L269-275 — `(&self, addr: &SocketAddr) -> Result<(), EgressDenied>` — IP literals.
-  `legacy_policy_hostname_dial_unchanged` function L279-298 — `()` — IP literals.
-  `map_resolver` function L301-316 — `(entries: &[(&str, IpAddr)]) -> TestResolver` — A resolver mapping fixed names to fixed IPs — the injected stand-in for DNS.
-  `same_ip_second_name_denied_unless_listed` function L323-359 — `()` — Pin correctness, not IP fallthrough (initiative criterion 3): two names
-  `unlisted_name_denied` function L364-385 — `()` — Unlisted name → denied (criterion 3, simple half): no allow-list entry, no
-  `literal_dial_to_pinned_ip_is_attributed_to_the_name` function L393-421 — `()` — Pin attribution: after `db.internal` resolves to an IP, a LITERAL dial to
-  `rotation_replaces_pin_and_stale_ip_loses_authority` function L431-490 — `()` — Rotation / resident lifetime (initiative criterion 4): within ONE store,
-  `mixed_case_dial_matches_lowercase_allow_list` function L496-523 — `()` — Case-insensitivity (DNS is case-insensitive): the guest dials
-  `unresolvable_name_fails_lookup_without_reaching_policy` function L528-552 — `()` — Unresolvable name: the resolver has no entry → the guest's lookup fails,
-  `multi_ip_resolution_pins_all_candidates` function L558-599 — `()` — Multi-address resolution: a name resolving to several IPs pins ALL of
-  `configured_instance_pins_persist_and_rotate_across_calls` function L607-715 — `()` — The REAL resident-instance path (initiative criterion 4, production
-  `builder_path_hostname_dial_authorized_by_name` function L725-792 — `()` — The ergonomic embedder path — `PluginHost::builder().egress(..)` +
-  `TCP_METHODS` variable L733-737 — `: [WasmMethodDesc; 1]` — IP literals.
-  `TCP_ECHO` variable L738-743 — `: WasmInterfaceDescriptor` — IP literals.
...
#### crates/fidius-host/tests/keyword_fields_wasm.rs

- pub `DeadLetter` struct L39-44 — `{ record: String, stream: u64, from: bool, r#type: u8 }` — generated conversions marshal a keyword-field record both ways.
- pub `Outcome` enum L47-51 — `Stream | Record | List` — generated conversions marshal a keyword-field record both ways.
- pub `Sink` interface L54-56 — `{ fn record() }` — generated conversions marshal a keyword-field record both ways.
-  `keyword_fields_component` function L58-72 — `() -> &'static [u8]` — generated conversions marshal a keyword-field record both ways.
-  `BYTES` variable L59 — `: OnceLock<Vec<u8>>` — generated conversions marshal a keyword-field record both ways.
-  `stage_pkg` function L74-96 — `(root: &std::path::Path)` — generated conversions marshal a keyword-field record both ways.
-  `keyword_field_record_round_trips` function L99-142 — `()` — generated conversions marshal a keyword-field record both ways.
...
-  `StrictSchema` struct L37-41 — `{ category: String, description: String, required_field: String }` — End-to-end package tests: validate, build, load, call.
-  `load_manifest_with_schema` function L44-54 — `()` — End-to-end package tests: validate, build, load, call.
-  `schema_mismatch_fails` function L57-67 — `()` — End-to-end package tests: validate, build, load, call.
-  `build_and_load_package` function L70-108 — `()` — End-to-end package tests: validate, build, load, call.
-  `AddInput` struct L97-100 — `{ a: i64, b: i64 }` — End-to-end package tests: validate, build, load, call.
-  `AddOutput` struct L102-104 — `{ result: i64 }` — End-to-end package tests: validate, build, load, call.
-  `discover_packages_finds_fixture` function L111-125 — `()` — End-to-end package tests: validate, build, load, call.
-  `missing_manifest_returns_error` function L128-132 — `()` — End-to-end package tests: validate, build, load, call.
...
#### crates/fidius-host/tests/python_bidi_stream_e2e.rs

- pub `Transformer` interface L30-32 — `{ fn transform() }` — one input item.
-  `transformer_descriptor` function L34-36 — `() -> &'static PythonInterfaceDescriptor` — one input item.
-  `repo_root` function L38-45 — `() -> PathBuf` — one input item.
-  `copy_dir` function L47-59 — `(src: &Path, dst: &Path)` — one input item.
-  `stage` function L61-77 — `(tmp: &tempfile::TempDir) -> PathBuf` — one input item.
-  `python_bidi_doubles_a_host_produced_stream` function L80-100 — `()` — one input item.
-  `python_bidi_pulls_lazily_from_an_unbounded_input` function L103-125 — `()` — one input item.
...
#### crates/fidius-host/tests/response_hook_e2e.rs

-  `IFACE` variable L41 — `: &str` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `fetcher_component` function L43-47 — `() -> Option<Vec<u8>>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `scripted_server` function L52-75 — `(responses: &'static [&'static str]) -> (String, mpsc::Receiver<String>)` — Scripted loopback server: serves `responses` in order, one connection
-  `load` function L77-102 — `(egress: Arc<dyn EgressPolicy>) -> Result<PluginHandle, CallError>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `RefreshOn401` struct L109-112 — `{ stamps: AtomicUsize, observations: Mutex<Vec<(u16, bool)>> }` — The motivating weir shape: `authorize` stamps the current credential
-  `RefreshOn401` type L114-121 — `= RefreshOn401` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `new` function L115-120 — `() -> Arc<Self>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `RefreshOn401` type L123-157 — `impl EgressPolicy for RefreshOn401` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `authorize` function L124-136 — `(&self, parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `observes_responses` function L137-139 — `(&self) -> bool` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `on_response` function L140-156 — `( &self, _request: &http::request::Parts, status: http::StatusCode, _headers: &h...` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `auth_retry_is_invisible_to_the_guest` function L163-189 — `()` — 401-then-200: the guest's single fetch succeeds; the server saw exactly
-  `RESPONSES` variable L168-171 — `: &[&str]` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `AlwaysRetry` struct L193 — `-` — An always-RetryOnce policy is bounded by fidius to a single retry: the
-  `AlwaysRetry` type L195-211 — `impl EgressPolicy for AlwaysRetry` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `authorize` function L196-198 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `observes_responses` function L199-201 — `(&self) -> bool` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `on_response` function L202-210 — `( &self, _request: &http::request::Parts, _status: http::StatusCode, _headers: &...` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `second_retry_directive_is_ignored` function L214-234 — `()` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `RESPONSES` variable L219-222 — `: &[&str]` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `DenyOnRetry` struct L239 — `-` — The policy consumes the 401 and then refuses to re-stamp: the guest gets
-  `DenyOnRetry` type L241-260 — `impl EgressPolicy for DenyOnRetry` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `authorize` function L242-247 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `observes_responses` function L248-250 — `(&self) -> bool` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `on_response` function L251-259 — `( &self, _request: &http::request::Parts, _status: http::StatusCode, _headers: &...` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `deny_on_retry_surfaces_as_denied` function L263-280 — `()` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `RESPONSES` variable L268-269 — `: &[&str]` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `AllowAll` struct L286 — `-` — A policy overriding neither `observes_responses` nor `on_response` rides
-  `AllowAll` type L288-292 — `impl EgressPolicy for AllowAll` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `authorize` function L289-291 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `policy_without_overrides_is_unaffected` function L295-308 — `()` — fixture for a constraint that is enforced below the guest boundary anyway.
-  `RESPONSES` variable L300-301 — `: &[&str]` — fixture for a constraint that is enforced below the guest boundary anyway.

#### crates/fidius-host/tests/tcp_egress_e2e.rs

-  `IFACE` variable L43 — `: &str` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_echo_component` function L49-68 — `() -> Option<Vec<u8>>` — Build the tcp-echo component once (mirrors `macro_egress_e2e`): CI installs the
-  `BYTES` variable L50 — `: OnceLock<Option<Vec<u8>>>` — embedder writes for a DB connector — fidius ships none of this.
-  `mock_tcp_echo_once` function L72-85 — `(n: usize) -> (String, std::thread::JoinHandle<()>)` — One-shot mock TCP echo server on an ephemeral loopback port: accepts one
-  `AllowLoopbackTcp` struct L88 — `-` — Reference embedder policy: allow TCP to loopback (the test's grant), deny HTTP.
-  `AllowLoopbackTcp` type L89-100 — `impl EgressPolicy for AllowLoopbackTcp` — embedder writes for a DB connector — fidius ships none of this.
-  `authorize` function L90-92 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — embedder writes for a DB connector — fidius ships none of this.
-  `authorize_tcp` function L93-99 — `(&self, addr: &SocketAddr) -> Result<(), EgressDenied>` — embedder writes for a DB connector — fidius ships none of this.
-  `DenyAllTcp` struct L103 — `-` — Reference embedder policy: deny all TCP (relies on the default-deny `authorize_tcp`).
-  `DenyAllTcp` type L104-109 — `impl EgressPolicy for DenyAllTcp` — embedder writes for a DB connector — fidius ships none of this.
-  `authorize` function L105-107 — `(&self, _parts: &mut http::request::Parts) -> Result<(), EgressDenied>` — embedder writes for a DB connector — fidius ships none of this.
-  `load` function L111-139 — `( caps: Vec<String>, egress: Option<Arc<dyn EgressPolicy>>, ) -> Result<PluginHa...` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_egress_allowed_echoes` function L142-158 — `()` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_egress_denied_by_policy` function L161-172 — `()` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_no_capability_fails_closed` function L175-187 — `()` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_no_policy_fails_closed` function L190-203 — `()` — embedder writes for a DB connector — fidius ships none of this.
-  `TCP_METHODS` variable L209-213 — `: [WasmMethodDesc; 1]` — embedder writes for a DB connector — fidius ships none of this.
-  `TCP_ECHO` variable L214-219 — `: WasmInterfaceDescriptor` — embedder writes for a DB connector — fidius ships none of this.
-  `stage_tcp_pkg` function L222-237 — `(root: &std::path::Path)` — Stage the tcp-echo fixture as a loadable wasm package declaring the `tcp` capability.
-  `tcp_egress_via_builder_default_policy` function L240-262 — `()` — embedder writes for a DB connector — fidius ships none of this.
-  `tcp_egress_via_per_plugin_policy` function L265-286 — `()` — embedder writes for a DB connector — fidius ships none of this.

#### crates/fidius-host/tests/wasm_bidi_stream_e2e.rs

- pub `Transformer` interface L34-36 — `{ fn transform() }` — which pulls one input item via the import (the synchronous lazy-pull composition).
-  `component` function L38-52 — `() -> &'static [u8]` — which pulls one input item via the import (the synchronous lazy-pull composition).
-  `BYTES` variable L39 — `: OnceLock<Vec<u8>>` — which pulls one input item via the import (the synchronous lazy-pull composition).
-  `stage_pkg` function L54-76 — `(root: &std::path::Path)` — which pulls one input item via the import (the synchronous lazy-pull composition).
-  `wasm_bidi_doubles_a_host_produced_stream` function L79-105 — `()` — which pulls one input item via the import (the synchronous lazy-pull composition).

#### crates/fidius-host/tests/wasm_client_stream_e2e.rs

- pub `Sink` interface L32-34 — `{ fn load() }` — + `WasmHostStream` + `call_client_streaming` over a real component.
-  `component` function L36-50 — `() -> &'static [u8]` — + `WasmHostStream` + `call_client_streaming` over a real component.
-  `BYTES` variable L37 — `: OnceLock<Vec<u8>>` — + `WasmHostStream` + `call_client_streaming` over a real component.
-  `stage_pkg` function L52-74 — `(root: &std::path::Path)` — + `WasmHostStream` + `call_client_streaming` over a real component.
-  `wasm_consumes_a_host_produced_stream` function L77-94 — `()` — + `WasmHostStream` + `call_client_streaming` over a real component.
...
#### crates/fidius-host/tests/wasm_grants_e2e.rs

- pub `ConfiguredFs` interface L47-49 — `{ fn read_file() }` — the manifest caps go through (the override is not a trust bypass).
-  `Cfg` struct L40-42 — `{ suffix: String }` — the manifest caps go through (the override is not a trust bypass).
-  `READ_FILE` variable L51 — `: usize` — the manifest caps go through (the override is not a trust bypass).
-  `component` function L53-67 — `() -> &'static [u8]` — the manifest caps go through (the override is not a trust bypass).
-  `BYTES` variable L54 — `: OnceLock<Vec<u8>>` — the manifest caps go through (the override is not a trust bypass).
-  `stage_pkg` function L72-106 — `(root: &Path, manifest_caps: &[String])` — Stage a `runtime = "wasm"` package whose manifest carries `manifest_caps`.
-  `host_for` function L108-116 — `(manifest_caps: &[String]) -> (tempfile::TempDir, PluginHost)` — the manifest caps go through (the override is not a trust bypass).
-  `load_time_grant_overrides_empty_manifest_and_config_is_bound` function L119-143 — `()` — the manifest caps go through (the override is not a trust bypass).
-  `empty_grant_denies_io_even_when_manifest_would_allow` function L146-170 — `()` — the manifest caps go through (the override is not a trust bypass).
-  `coarse_grant_is_rejected_at_load` function L173-193 — `()` — the manifest caps go through (the override is not a trust bypass).

#### crates/fidius-host/tests/wasm_host_functions_e2e.rs

- pub `WasmSlotHost` interface L42-47 — `{ fn release_slot(), fn reclaim_slot(), fn get_value(), fn panicky() }` — binds (or doesn't) its own fresh instance.
- pub `WasmDeferrable` interface L50-55 — `{ fn run(), fn host_bound(), fn try_release(), fn observe_host_panic() }` — binds (or doesn't) its own fresh instance.
-  `component` function L57-71 — `() -> &'static [u8]` — binds (or doesn't) its own fresh instance.
-  `BYTES` variable L58 — `: OnceLock<Vec<u8>>` — binds (or doesn't) its own fresh instance.
-  `stage_pkg` function L73-95 — `(root: &std::path::Path)` — binds (or doesn't) its own fresh instance.
-  `load_handle` function L98-109 — `(tmp: &tempfile::TempDir) -> PluginHandle` — Load a fresh handle over the fixture component (own table registry).
-  `RecordingHost` struct L113-116 — `{ calls: Mutex<Vec<String>>, reclaims: AtomicU32 }` — Recording host implementation shared by the tests.
-  `RecordingHost` type L118-144 — `impl WasmSlotHost for RecordingHost` — binds (or doesn't) its own fresh instance.
-  `release_slot` function L119-129 — `(&self, task_id: String) -> Result<(), PluginError>` — binds (or doesn't) its own fresh instance.
-  `reclaim_slot` function L130-136 — `(&self, task_id: String) -> Result<u32, PluginError>` — binds (or doesn't) its own fresh instance.
-  `get_value` function L137-140 — `(&self, key: String) -> String` — binds (or doesn't) its own fresh instance.
-  `panicky` function L141-143 — `(&self) -> u32` — binds (or doesn't) its own fresh instance.
-  `table` function L147-149 — `(host: Arc<RecordingHost>) -> *const HostFunctionTable` — Build a process-lifetime table for a recording host.
-  `wasm_reentrant_host_plugin_host_roundtrip` function L152-177 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_host_error_arrives_typed` function L180-197 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_host_panic_surfaces_as_typed_error` function L200-214 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_unbound_is_a_typed_error_not_a_trap` function L217-235 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_version_mismatch_is_typed_and_never_dispatches` function L238-269 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_hash_mismatch_is_typed_and_never_dispatches` function L272-291 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_second_bind_is_refused` function L294-311 — `()` — binds (or doesn't) its own fresh instance.
-  `wasm_bind_on_a_cdylib_handle_is_refused` function L314-328 — `()` — binds (or doesn't) its own fresh instance.
...
#### crates/fidius-host/tests/wasm_record_stream_user_types.rs

- pub `Row` struct L33-36 — `{ id: u64, label: String }` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
- pub `Pipe` interface L39-43 — `{ fn rows(), fn count_from(), fn ingest() }` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `component` function L45-62 — `() -> &'static [u8]` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `BYTES` variable L46 — `: OnceLock<Vec<u8>>` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `stage_pkg` function L64-86 — `(root: &std::path::Path)` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `load` function L88-97 — `() -> fidius_host::PluginHandle` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `wasm_bidi_record_output` function L100-128 — `()` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `wasm_client_with_user_typed_arg` function L131-142 — `()` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
-  `wasm_client_record_stream_item` function L145-166 — `()` — branch's new client/bidi codegen + the build.rs WIT (which skips the stream arg).
...
#### crates/fidius-macro/src/host_interface.rs

- pub `HostInterfaceAttrs` struct L50-54 — `{ version: u32, crate_path: Path }` — Parsed attributes from `#[host_interface(version = N)]`.
- pub `generate_host_interface` function L273-834 — `( attrs: &HostInterfaceAttrs, item: &ItemTrait, ) -> syn::Result<TokenStream>` — Generate all code for a `#[host_interface]` invocation.
-  `HostInterfaceAttrs` type L56-98 — `impl Parse for HostInterfaceAttrs` — for the threading/reentrancy contract.
-  `parse` function L57-97 — `(input: ParseStream) -> syn::Result<Self>` — for the threading/reentrancy contract.
-  `HostMethod` struct L101-110 — `{ name: Ident, arg_names: Vec<Ident>, arg_types: Vec<Type>, return_type: Option<...` — IR for a single host-function method.
-  `last_segment` function L113-118 — `(ty: &Type) -> Option<&syn::PathSegment>` — Return `Some(&last_segment)` if `ty` is a path type.
-  `result_types` function L121-134 — `(ty: &Type) -> Option<(&Type, &Type)>` — If `ty` is `Result<T, E>`, return `(T, E)`.
-  `is_stream_marker` function L138-145 — `(ty: &Type) -> bool` — Return `true` if the type's final path segment is `Stream` with one
-  `parse_method` function L148-270 — `(method: &TraitItemFn) -> syn::Result<HostMethod>` — Parse and validate one trait method into a [`HostMethod`].
-  `strip_helper_attrs` function L837-847 — `(item: &ItemTrait) -> ItemTrait` — Strip fidius helper attributes so the emitted trait compiles bare.
-  `tests` module L850-978 — `-` — for the threading/reentrancy contract.
-  `attrs` function L854-859 — `(version: u32) -> HostInterfaceAttrs` — for the threading/reentrancy contract.
-  `parse_trait` function L861-863 — `(tokens: proc_macro2::TokenStream) -> ItemTrait` — for the threading/reentrancy contract.
-  `generates_for_a_simple_host_trait` function L866-879 — `()` — for the threading/reentrancy contract.
-  `rejects_async_methods` function L882-890 — `()` — for the threading/reentrancy contract.
-  `rejects_missing_send_sync` function L893-901 — `()` — for the threading/reentrancy contract.
-  `rejects_non_plugin_error_result` function L904-912 — `()` — for the threading/reentrancy contract.
-  `rejects_reference_arguments` function L915-923 — `()` — for the threading/reentrancy contract.
-  `rejects_stream_marker` function L926-934 — `()` — for the threading/reentrancy contract.
-  `rejects_mut_self` function L937-945 — `()` — for the threading/reentrancy contract.
-  `hash_is_signature_sensitive` function L948-966 — `()` — for the threading/reentrancy contract.
-  `attrs_parse_version_and_crate` function L969-977 — `()` — for the threading/reentrancy contract.
...
-  `generate_wasm_adapter` function L357-951 — `( trait_name: &Ident, instance_name: &Ident, methods: &[MethodInfo], config: Opt...` — Generate the WASM component auto-export adapter for `#[plugin_impl]`.
-  `collect_user_idents` function L955-1000 — `(ty: &Type, out: &mut std::collections::BTreeSet<String>)` — Collect candidate user-type idents (non-primitive path leaves) from a type,
-  `gen_type` function L1005-1045 — `(ty: &Type, known: &std::collections::BTreeSet<String>, pkg_seg: &Ident) -> Toke...` — The wit-bindgen-generated type for an author type: identity for types holding
-  `wasm_first_generic` function L1047-1056 — `(seg: &syn::PathSegment) -> Option<&Type>` — dylibs, the FIDIUS_PLUGIN_REGISTRY.
-  `wasm_two_generics` function L1058-1073 — `(seg: &syn::PathSegment) -> Option<(&Type, &Type)>` — dylibs, the FIDIUS_PLUGIN_REGISTRY.
-  `wasm_unsupported` function L1079-1089 — `(method: &Ident, reason: &str) -> TokenStream` — Emit a `#[cfg(target_family = "wasm")]`-gated `compile_error!` for a method
-  `generate_shims` function L1093-1599 — `( impl_ident: &Ident, methods: &[MethodInfo], crate_path: &Path, buffer_strategy...` — Generate extern "C" shim functions for each method.
-  `generate_vtable_static` function L1605-1627 — `( trait_name: &Ident, impl_ident: &Ident, methods: &[&Ident], ) -> TokenStream` — Generate the static vtable with function pointers.
-  `generate_descriptor` function L1630-1756 — `( trait_name: &Ident, impl_ident: &Ident, methods: &[&Ident], crate_path: &Path,...` — Generate the PluginDescriptor static.
-  `generate_inventory_registration` function L1759-1770 — `(impl_ident: &Ident, crate_path: &Path) -> TokenStream` — Register the descriptor via inventory for multi-plugin support.
...
- pub `parse_interface` function L499-595 — `(attrs: InterfaceAttrs, item: &ItemTrait) -> syn::Result<InterfaceIR>` — Parse an `ItemTrait` into an `InterfaceIR`.
...
-  `build_signature_string` function L413-455 — `( method: &TraitItemFn, wire_raw: bool, stream_item: Option<&Type>, client_strea...` — Build the canonical signature string for a method.
-  `extract_arg_names` function L458-475 — `(method: &TraitItemFn) -> Vec<Ident>` — Extract argument names from a method signature (excluding `self`).
-  `extract_arg_types` function L478-488 — `(method: &TraitItemFn) -> Vec<Type>` — Extract argument types from a method signature (excluding `self`).
-  `extract_return_type` function L491-496 — `(method: &TraitItemFn) -> Option<Type>` — Extract the return type (unwrapped from `-> Type`).
-  `tests` module L598-846 — `-` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `parse_test_trait` function L602-610 — `(tokens: proc_macro2::TokenStream) -> InterfaceIR` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `basic_trait_parsing` function L613-630 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `optional_method_parsing` function L633-646 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `async_method_detection` function L649-659 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `rejects_mut_self` function L662-678 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `signature_string_format` function L681-691 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `interface_attrs_parsing` function L694-700 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `interface_attrs_with_crate_path` function L703-716 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `detects_server_streaming_return` function L719-738 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `streaming_and_unary_hash_differently` function L741-755 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `bare_stream_marker_is_detected` function L758-765 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `client_streaming_is_recognized_in_the_ir` function L768-793 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `is_required_reflects_optional_attr` function L799-812 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `is_vec_u8_distinguishes_types` function L815-820 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `extract_arg_names_returns_each_non_self_arg` function L823-830 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.
-  `validate_raw_method_signature_rejects_bad_shapes` function L833-845 — `()` — Both `#[plugin_interface]` and `#[plugin_impl]` consume this IR.

#### crates/fidius-macro/src/lib.rs

- pub `plugin_interface` function L45-56 — `(attr: TokenStream, item: TokenStream) -> TokenStream` — Define a plugin interface from a trait.
- pub `host_interface` function L155-163 — `(attr: TokenStream, item: TokenStream) -> TokenStream` — Define a **host interface** from a trait — the plugin → host callback
- pub `plugin_impl` function L183-191 — `(attr: TokenStream, item: TokenStream) -> TokenStream` — Implement a plugin interface for a concrete type.
- pub `derive_wit_type` function L207-211 — `(_item: TokenStream) -> TokenStream` — Mark a `struct`/`enum` as usable in a WASM plugin interface (FIDIUS-I-0023).
-  `host_interface` module L15 — `-`
-  `impl_macro` module L16 — `-`
-  `interface` module L17 — `-`
-  `ir` module L18 — `-`
-  `wit` module L19 — `-`
...
- pub `call_client_streaming_json` function L165-211 — `( &self, method_index: usize, items: Box<dyn Iterator<Item = serde_json::Value> ...` — **Client-streaming** call (FIDIUS-I-0030 CS2.4): the host produces the stream
- pub `call_bidi_streaming_start` function L218-269 — `( &self, method_index: usize, items: Box<dyn Iterator<Item = serde_json::Value> ...` — **Bidirectional** streaming (FIDIUS-I-0032 / ADR-0010): the method receives the
- pub `call_streaming_start` function L275-300 — `( &self, method_index: usize, input_json: &[u8], ) -> Result<crate::stream::Pyth...` — Start a server-streaming call (FIDIUS-I-0026).
- pub `call_raw` function L303-322 — `(&self, method_index: usize, input: &[u8]) -> Result<Vec<u8>, PythonCallError>` — Raw dispatch — pass bytes in, get bytes out, no encoding.
-  `PythonPluginHandle` type L84-347 — `= PythonPluginHandle` — `code = <ExceptionClassName>` otherwise.
...
-  `lookup_method` function L324-346 — `( &self, index: usize, attempting_raw: bool, ) -> Result<MethodLookup<'_>, Pytho...` — `code = <ExceptionClassName>` otherwise.
-  `MethodLookup` struct L349-351 — `{ callable: &'a Py<PyAny> }` — `code = <ExceptionClassName>` otherwise.
-  `build_call_args` function L360-379 — `( py: Python<'py>, input: &serde_json::Value, ) -> PyResult<Bound<'py, PyTuple>>` — Build positional args for `callable.call(...)` from a JSON value.
-  `HostFedStream` struct L385-391 — `{ items: std::sync::Mutex<Box<dyn Iterator<Item = serde_json::Value> + Send>> }` — A host-fed Python iterator (FIDIUS-I-0030 CS2.4): yields the host's stream items
-  `HostFedStream` type L394-406 — `= HostFedStream` — `code = <ExceptionClassName>` otherwise.
-  `__iter__` function L395-397 — `(slf: PyRef<'_, Self>) -> PyRef<'_, Self>` — `code = <ExceptionClassName>` otherwise.
-  `__next__` function L399-405 — `(&self, py: Python<'_>) -> PyResult<Option<Py<PyAny>>>` — `code = <ExceptionClassName>` otherwise.
...
- pub `conv_expr` function L398-443 — `(access: &str, ty: &Type, known: &BTreeSet<String>) -> String` — Conversion expression for a field/payload `access` of type `ty`.
- pub `contains_user_type` function L447-467 — `(ty: &Type, known: &BTreeSet<String>) -> bool` — Whether `ty` is, or contains (through `Vec`/`Option`/`Box`), a user type in
...
-  `assemble` function L130-214 — `(acc: Collected) -> Result<Generated, String>` — Build the `.wit` + conversions from the collected items.
-  `author_path` function L217-223 — `(mod_path: &[String], name: &str) -> String` — `crate::<mod::path>::<Name>` — the author-side path for a type at `mod_path`.
-  `RUST_KEYWORDS` variable L231-237 — `: &[&str]` — The keywords wit-bindgen's `to_rust_ident` escapes with a trailing `_` (its
-  `field_idents` function L244-253 — `(fl: &syn::Field) -> (String, String)` — The author's field ident (`record`, or the raw `r#type`) paired with the name
-  `render_conversions` function L259-390 — `( iface_kebab: &str, structs: &[(Vec<String>, syn::ItemStruct)], enums: &[(Vec<S...` — Render `From` impls (both directions) between each user type and its
-  `single_generic` function L469-478 — `(seg: &syn::PathSegment) -> Option<&Type>` — the `fidius wit` CLI, which read the source files.
-  `two_generics` function L480-495 — `(seg: &syn::PathSegment) -> Option<(&Type, &Type)>` — the `fidius wit` CLI, which read the source files.
-  `has_attr` function L498-506 — `(attrs: &[syn::Attribute], name: &str) -> bool` — Does `attrs` contain `#[<name>(...)]` / `#[<path>::<name>]` (last segment match)?
-  `has_derive` function L509-531 — `(attrs: &[syn::Attribute], name: &str) -> bool` — Does `attrs` contain a `#[derive(...
-  `tests` module L534-676 — `-` — the `fidius wit` CLI, which read the source files.
-  `SRC` variable L537-550 — `: &str` — the `fidius wit` CLI, which read the source files.
-  `generates_wit_with_records_variants_and_funcs` function L553-569 — `()` — the `fidius wit` CLI, which read the source files.
-  `generates_conversions_both_ways` function L572-585 — `()` — the `fidius wit` CLI, which read the source files.
-  `primitive_only_interface_has_no_conversions` function L588-597 — `()` — the `fidius wit` CLI, which read the source files.
-  `unsupported_type_errors` function L600-608 — `()` — the `fidius wit` CLI, which read the source files.
-  `KEYWORD_SRC` variable L615-626 — `: &str` — FIDIUS-T-0177: a guest interface whose types, fields, cases, methods, and
-  `keyword_heavy_interface_escapes_every_identifier` function L629-652 — `()` — the `fidius wit` CLI, which read the source files.
-  `keyword_field_conversions_use_wit_bindgen_mangling` function L655-666 — `()` — the `fidius wit` CLI, which read the source files.
-  `keyword_heavy_interface_parses` function L669-675 — `()` — the `fidius wit` CLI, which read the source files.

#### crates/fidius-wit/src/lib.rs

- pub `to_kebab_case` function L42-58 — `(s: &str) -> String` — Convert a Rust identifier (CamelCase or snake_case) to kebab-case, the WIT
- pub `is_wit_keyword` function L116-118 — `(ident: &str) -> bool` — Whether `ident` (an already-kebab-cased WIT identifier) is a reserved WIT
- pub `wit_ident` function L129-135 — `(ident: &str) -> String` — Escape a generated WIT identifier so it is valid even when it collides with a
- pub `result_ok_type` function L138-147 — `(ty: &Type) -> Option<&Type>` — Extract the `T` from `Result<T, _>`, if `ty` is a `Result`.
- pub `WitMethod` struct L150-163 — `{ name: String, params: Vec<(String, String)>, ret: Option<String>, stream_item:...` — One method projected to WIT (already-mapped strings).
- pub `stream_item_type` function L168-177 — `(ty: &Type) -> Option<&Type>` — If `ty` is `fidius::Stream<T>` (final path segment `Stream`, exactly one type
- pub `wit_type_with` function L182-262 — `(ty: &Type, known: &BTreeSet<String>) -> Result<String, String>` — Map a Rust argument/return type to its WIT spelling, where `known` holds the
- pub `rust_type_to_wit` function L266-268 — `(ty: &Type) -> Result<String, String>` — Primitive/std-only mapping (no user types) — the form `fidius-macro` uses for
- pub `return_to_wit_with` function L273-295 — `( ret: Option<&Type>, known: &BTreeSet<String>, ) -> Result<Option<String>, Stri...` — Map a method's return type to an optional WIT return, with user types in
- pub `return_to_wit` function L298-300 — `(ret: Option<&Type>) -> Result<Option<String>, String>` — Primitive/std-only return mapping (no user types).
- pub `struct_to_record` function L305-322 — `(item: &ItemStruct, known: &BTreeSet<String>) -> Result<String, String>` — Render a `record <name> { ...
- pub `enum_to_wit` function L332-378 — `( item: &ItemEnum, known: &BTreeSet<String>, ) -> Result<(Vec<String>, String), ...` — Render a Rust enum to WIT: a `variant <name> { ...
- pub `render_wit_full` function L384-446 — `(iface_kebab: &str, type_defs: &[String], methods: &[WitMethod]) -> String` — Render a complete `.wit` document: package + interface (the `plugin-error`
- pub `render_wit` function L450-452 — `(iface_kebab: &str, methods: &[WitMethod]) -> String` — Convenience: render a WIT document with no user type defs (the primitives-only
-  `generate` module L30 — `-` — helper, and the `fidius wit` CLI can all share one implementation.
-  `WIT_KEYWORDS` variable L69-112 — `: &[&str]` — The reserved keywords of the WIT grammar (mirrors wit-parser 0.236's
-  `is_unit` function L456-458 — `(ty: &Type) -> bool` — helper, and the `fidius wit` CLI can all share one implementation.
-  `path_is` function L460-466 — `(p: &syn::TypePath, name: &str) -> bool` — helper, and the `fidius wit` CLI can all share one implementation.
-  `single_generic` function L468-470 — `(seg: &'a syn::PathSegment, what: &str) -> Result<&'a Type, String>` — helper, and the `fidius wit` CLI can all share one implementation.
-  `first_generic` function L472-481 — `(seg: &syn::PathSegment) -> Option<&Type>` — helper, and the `fidius wit` CLI can all share one implementation.
-  `two_generics` function L484-499 — `(seg: &'a syn::PathSegment, what: &str) -> Result<(&'a Type, &'a Type), String>` — Extract the first two type arguments (e.g.
-  `tests` module L502-736 — `-` — helper, and the `fidius wit` CLI can all share one implementation.
-  `known` function L505-507 — `(names: &[&str]) -> BTreeSet<String>` — helper, and the `fidius wit` CLI can all share one implementation.
-  `wit` function L508-510 — `(s: &str) -> String` — helper, and the `fidius wit` CLI can all share one implementation.
-  `primitives_strings_containers` function L513-521 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `maps_tuples_and_nesting` function L524-544 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `returns` function L547-558 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `user_types_need_the_known_set` function L561-574 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `struct_renders_to_record` function L577-583 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `struct_with_nested_user_type` function L586-591 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `enum_renders_to_variant` function L594-603 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `struct_variant_synthesizes_a_record` function L606-615 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `multifield_tuple_variant_is_rejected` function L618-621 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `full_document_places_type_defs_before_funcs` function L624-647 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `streaming_method_renders_a_resource` function L650-665 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `keyword_idents_are_escaped_others_untouched` function L668-676 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `raw_idents_lose_their_prefix_then_escape` function L679-686 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `record_with_keyword_fields_escapes_field_and_record_names` function L689-700 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `variant_with_keyword_cases_escapes_them` function L703-715 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `keyword_user_type_reference_matches_its_declaration` function L718-724 — `()` — helper, and the `fidius wit` CLI can all share one implementation.
-  `stream_item_type_detects_marker` function L727-735 — `()` — helper, and the `fidius wit` CLI can all share one implementation.

### crates/fidius-wit/tests

> *Semantic summary to be generated by AI agent.*

#### crates/fidius-wit/tests/proptest_wit.rs

-  `TyModel` enum L29-47 — `Bool | I8 | I16 | I32 | I64 | U8 | U16 | U32 | U64 | F32 | F64 | Char | Str | Ve...` — A type the WASM interface mapping supports.
-  `TyModel` type L49-100 — `= TyModel` — the supported types (primitives, `Vec`, `Option`, maps, tuples).
-  `rust` function L50-74 — `(&self) -> String` — the supported types (primitives, `Vec`, `Option`, maps, tuples).
-  `wit` function L76-99 — `(&self) -> String` — the supported types (primitives, `Vec`, `Option`, maps, tuples).
-  `arb_tymodel` function L102-128 — `() -> impl Strategy<Value = TyModel>` — the supported types (primitives, `Vec`, `Option`, maps, tuples).
...
#### examples/examples/06_bidi_transform.rs

- pub `Transformer` interface L15-18 — `{ fn transform() }` — Run: `cargo run -p fidius-examples --example 06_bidi_transform`
- pub `Doubler` struct L20 — `-` — Run: `cargo run -p fidius-examples --example 06_bidi_transform`
-  `Doubler` type L23-29 — `impl Transformer for Doubler` — Run: `cargo run -p fidius-examples --example 06_bidi_transform`
-  `transform` function L24-28 — `(&self, mut input: Stream<u64>) -> Stream<u64>` — Run: `cargo run -p fidius-examples --example 06_bidi_transform`
-  `main` function L34-51 — `()` — Run: `cargo run -p fidius-examples --example 06_bidi_transform`

#### examples/examples/07_client_streaming.rs

- pub `Sink` interface L13-16 — `{ fn sum() }` — Run: `cargo run -p fidius-examples --example 07_client_streaming`
- pub `Adder` struct L18 — `-` — Run: `cargo run -p fidius-examples --example 07_client_streaming`
-  `Adder` type L21-29 — `impl Sink for Adder` — Run: `cargo run -p fidius-examples --example 07_client_streaming`
-  `sum` function L22-28 — `(&self, mut rows: Stream<u64>) -> u64` — Run: `cargo run -p fidius-examples --example 07_client_streaming`
-  `main` function L33-43 — `()` — Run: `cargo run -p fidius-examples --example 07_client_streaming`

#### examples/examples/08_host_functions.rs

- pub `SlotHost` interface L22-29 — `{ fn release_slot(), fn reclaim_slot(), fn free_slots() }` — Run: `cargo run -p fidius-examples --example 08_host_functions`
- pub `Task` interface L34-36 — `{ fn execute() }` — Run: `cargo run -p fidius-examples --example 08_host_functions`
- pub `DeferringTask` struct L38 — `-` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `DeferringTask` type L41-61 — `impl Task for DeferringTask` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `execute` function L42-60 — `(&self, task_id: String) -> Result<String, PluginError>` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `Executor` struct L67-69 — `{ free: AtomicI64 }` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `Executor` type L71-85 — `impl SlotHost for Executor` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `release_slot` function L72-76 — `(&self, task_id: String) -> Result<(), PluginError>` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `reclaim_slot` function L77-81 — `(&self, task_id: String) -> Result<(), PluginError>` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `free_slots` function L82-84 — `(&self) -> i64` — Run: `cargo run -p fidius-examples --example 08_host_functions`
-  `main` function L87-109 — `()` — Run: `cargo run -p fidius-examples --example 08_host_functions`
...
### tests/test-plugin-hostcall/src

> *Semantic summary to be generated by AI agent.*

#### tests/test-plugin-hostcall/src/lib.rs

- pub `TestHost` interface L30-45 — `{ fn release_slot(), fn reclaim_slot(), fn get_value(), fn panicky() }` — The host functions this plugin can call back into.
- pub `Deferrable` interface L49-71 — `{ fn run_deferred(), fn value_from_thread(), fn observe_host_panic(), fn host_bo...` — The plugin interface the host drives.
- pub `DeferrablePlugin` struct L73 — `-` — host → plugin → host path end to end.
-  `DeferrablePlugin` type L76-131 — `impl Deferrable for DeferrablePlugin` — host → plugin → host path end to end.
-  `run_deferred` function L77-93 — `(&self, task_id: String) -> Result<String, PluginError>` — host → plugin → host path end to end.
-  `value_from_thread` function L95-102 — `(&self, key: String) -> String` — host → plugin → host path end to end.
-  `observe_host_panic` function L104-110 — `(&self) -> String` — host → plugin → host path end to end.
-  `host_bound` function L112-114 — `(&self) -> bool` — host → plugin → host path end to end.
-  `try_release` function L116-124 — `(&self, task_id: String) -> String` — host → plugin → host path end to end.
-  `panic_after_callback` function L126-130 — `(&self, task_id: String) -> String` — host → plugin → host path end to end.
...
- pub `Greeter` interface L134-136 — `{ fn greet() }`
- pub `GreetConfig` struct L139-141 — `{ greeting: String }`
- pub `ConfiguredGreeter` struct L143-145 — `{ cfg: GreetConfig }`
- pub `Ticker` interface L168-171 — `{ fn tick() }`
- pub `TickerImpl` struct L176 — `-`
...
-  `ConfiguredGreeter` type L148-153 — `impl Greeter for ConfiguredGreeter`
-  `greet` function L149-152 — `(&self, name: String) -> String`
-  `ConfiguredGreeter` type L155-159 — `= ConfiguredGreeter`
-  `configure` function L156-158 — `(cfg: GreetConfig) -> Self`
-  `TickerImpl` type L179-183 — `impl Ticker for TickerImpl`
-  `tick` function L180-182 — `(&self, count: u32) -> fidius::Stream<u64>`

### tests/wasm-fixtures/bidi-stream/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/bidi-stream/src/lib.rs

- pub `Transformer` interface L11-13 — `{ fn transform() }`
- pub `Doubler` struct L15 — `-`
-  `Doubler` type L18-24 — `impl Transformer for Doubler`
-  `transform` function L19-23 — `(&self, mut input: fidius_guest::Stream<u64>) -> fidius_guest::Stream<u64>`

### tests/wasm-fixtures/client-stream/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/client-stream/src/lib.rs

- pub `Sink` interface L11-14 — `{ fn load() }`
- pub `MySink` struct L16 — `-`
-  `MySink` type L19-27 — `impl Sink for MySink`
-  `load` function L20-26 — `(&self, mut rows: fidius_guest::Stream<u64>) -> u64`
...
### tests/wasm-fixtures/hostcall/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/hostcall/src/lib.rs

- pub `WasmSlotHost` interface L13-22 — `{ fn release_slot(), fn reclaim_slot(), fn get_value(), fn panicky() }`
- pub `WasmDeferrable` interface L25-34 — `{ fn run(), fn host_bound(), fn try_release(), fn observe_host_panic() }`
- pub `WasmDeferrablePlugin` struct L36 — `-`
-  `WasmDeferrablePlugin` type L39-81 — `impl WasmDeferrable for WasmDeferrablePlugin`
-  `run` function L40-56 — `(&self, task_id: String) -> String`
-  `host_bound` function L58-60 — `(&self) -> bool`
-  `try_release` function L62-70 — `(&self, task_id: String) -> String`
-  `observe_host_panic` function L72-80 — `(&self) -> String`

### tests/wasm-fixtures/keyword-fields

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/keyword-fields/build.rs

-  `main` function L7-9 — `()`

### tests/wasm-fixtures/keyword-fields/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/keyword-fields/src/lib.rs

- pub `DeadLetter` struct L16-21 — `{ record: String, stream: u64, from: bool, r#type: u8 }`
- pub `Outcome` enum L27-31 — `Stream | Record | List`
- pub `Sink` interface L34-38 — `{ fn record() }`
- pub `MySink` struct L40 — `-`
-  `MySink` type L43-55 — `impl Sink for MySink`
-  `record` function L44-54 — `(&self, list: DeadLetter, option: Outcome) -> DeadLetter`
...
### tests/wasm-fixtures/macro-configured-fs/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/macro-configured-fs/src/lib.rs

- pub `Cfg` struct L15-17 — `{ suffix: String }`
- pub `ConfiguredFs` interface L20-24 — `{ fn read_file() }`
- pub `ConfFsPlugin` struct L26-28 — `{ cfg: Cfg }`
-  `ConfFsPlugin` type L31-36 — `impl ConfiguredFs for ConfFsPlugin`
-  `read_file` function L32-35 — `(&self, path: String) -> String`
-  `ConfFsPlugin` type L38-42 — `= ConfFsPlugin`
-  `configure` function L39-41 — `(cfg: Cfg) -> Self`
...
### tests/wasm-fixtures/record-stream-user-types

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/record-stream-user-types/build.rs

-  `main` function L15-17 — `()`

### tests/wasm-fixtures/record-stream-user-types/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/record-stream-user-types/src/lib.rs

- pub `Row` struct L11-14 — `{ id: u64, label: String }`
- pub `Pipe` interface L17-24 — `{ fn rows(), fn count_from(), fn ingest() }`
- pub `MyPipe` struct L26 — `-`
-  `MyPipe` type L29-54 — `impl Pipe for MyPipe`
-  `rows` function L30-37 — `(&self, mut ids: fidius_guest::Stream<u64>) -> fidius_guest::Stream<Row>`
-  `count_from` function L39-45 — `(&self, mut ids: fidius_guest::Stream<u64>, base: Row) -> u64`
-  `ingest` function L47-53 — `(&self, mut rows: fidius_guest::Stream<Row>) -> u64`
...
### tests/wasm-fixtures/tcp-echo/src

> *Semantic summary to be generated by AI agent.*

#### tests/wasm-fixtures/tcp-echo/src/lib.rs

-  `Component` struct L24 — `-`
-  `Component` type L26-50 — `impl Guest for Component`
-  `connect_and_echo` function L27-29 — `(addr: String, payload: Vec<u8>) -> Vec<u8>`
-  `connect_seq` function L33-38 — `(addrs: Vec<String>, payload: Vec<u8>) -> Vec<Vec<u8>>` — FIDIUS-I-0034: sequential dials in ONE instance, so all dials share
-  `fidius_configure` function L43 — `(_cfg: Vec<u8>)` — No-op: this fixture has no config, but exporting the binder lets the
-  `fidius_interface_hash` function L47-49 — `() -> u64` — Interface-hash carrier; the host's `load_wasm` checks it against the
-  `do_echo` function L52-59 — `(addr: &str, payload: &[u8]) -> std::io::Result<Vec<u8>>`
