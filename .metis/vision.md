...
# Fides — Generalized Rust Plugin Framework

## Purpose

Fides extracts the trait → cFFI → dylib → dynamic loading pattern from cloacina into a standalone Rust library. Plugin authors define a trait, annotate it, and get a compiled dylib with a stable C ABI. Host applications load, validate, and call plugins through a type-safe proxy — no handwritten FFI.
...
- A CLI (`fides-cli`) for scaffolding interface/plugin crates, signing, verification, and inspection
- Interface evolution via optional methods and compile-time hash checking

**Key differentiator**: The plugin author writes normal Rust traits and impls. Fides generates all the unsafe FFI machinery, serialization, panic catching, and buffer management. The CLI scaffolds the correct crate topology so plugin authors get a single-dependency experience.
...
A published crate ecosystem (`fides`, `fides-core`, `fides-macro`, `fides-host`, `fides-cli`) that any Rust project can depend on to add a plugin system with:
...
- A non-cloacina Rust project can define, compile, sign, load, and call a plugin using only fides crates
...
4. **Plugin side stays light** — `fides-core` is the only dependency for plugins. No libloading, no signing verification, no host machinery.
