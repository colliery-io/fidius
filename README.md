...
- **Host functions (plugin → host callbacks)** — declare a `#[host_interface(version = N)]` trait the host implements and plugins call **mid-execution** (e.g. release/reclaim a concurrency slot around a wait). Same bincode wire, its own load-time version + signature-hash gate, panic-safe in both directions, typed errors. Dylib plugins in v1; see `docs/explanation/host-functions.md`.
