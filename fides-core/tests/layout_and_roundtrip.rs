//! Layout assertion tests and serialization round-trip tests for fides-core.
//!
...
use fides_core::error::PluginError;
use fides_core::hash::{fnv1a, interface_hash};
use fides_core::status::*;
use fides_core::wire;
...
fn hash_const_fnv1a() {
    ...
    const HASH: u64 = fnv1a(b"fides");
    assert_ne!(HASH, 0);
    assert_eq!(HASH, fnv1a(b"fides"));
}
...
fn magic_bytes_value() {
    assert_eq!(&FIDES_MAGIC, b"FIDES\0\0\0");
    assert_eq!(FIDES_MAGIC.len(), 8);
}
