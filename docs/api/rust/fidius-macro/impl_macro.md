...
| `wire_raw` | `bool` | Whether the impl method carries `#[wire(raw)]`. Must match the
...
| `ret_type` | `Option < & 'a Type >` | The method's return type, if any (`None` for `-> ()`). Used by the
WASM component WIT generator (FIDIUS-T-0106). |
| `stream_item` | `Option < & 'a Type >` | For a server-streaming method (`-> fidius::Stream<T>`): the item type `T`
...
| `client_stream_item` | `Option < & 'a Type >` | For a **client-streaming** method — a `Stream<T>` in *argument* position
...
| `config` | `Option < Path >` | FIDIUS-A-0006: the config type `C` for a configured plugin
...
### `fidius-macro::impl_macro::impl_method_is_raw`
...
Detect a `#[wire(raw)]` attribute on an impl-side method. Mirrors the trait-side parser in `ir.rs` — an explicit `raw` keyword is the only supported form.
...
### `fidius-macro::impl_macro::kebab_to_pascal`
...
kebab-case → PascalCase, for deriving the wit-bindgen resource type name from a method name: `tick` → resource `tick-stream` → `TickStream` (trait `GuestTickStream`). (FIDIUS-I-0026.)
...
### `fidius-macro::impl_macro::generate_wasm_adapter`

<span class="plissken-badge plissken-badge-visibility" style="display: inline-block; padding: 0.1em 0.35em; font-size: 0.55em; font-weight: 600; border-radius: 0.2em; vertical-align: middle; background: var(--md-default-fg-color--light); color: white;">private</span>


```rust
fn generate_wasm_adapter (trait_name : & Ident , instance_name : & Ident , methods : & [MethodInfo] , config : Option < & Path > , crate_path : & Path , impl_type : & Type ,) -> TokenStream
```
...
### `fidius-macro::impl_macro::collect_user_idents`
...
Collect candidate user-type idents (non-primitive path leaves) from a type, descending through `Vec`/`Option`/`Box` and `Result`'s ok type.
...
### `fidius-macro::impl_macro::gen_type`
...
The wit-bindgen-generated type for an author type: identity for types holding no user type (wit-bindgen uses the same Rust type), else the generated path `exports::fidius::<iface>::<iface>::<T>`, recursing through `Vec`/`Option`.
...
### `fidius-macro::impl_macro::wasm_first_generic`
...
### `fidius-macro::impl_macro::wasm_two_generics`
...
### `fidius-macro::impl_macro::wasm_unsupported`
...
Emit a `#[cfg(target_family = "wasm")]`-gated `compile_error!` for a method the WASM auto-export can't handle. On native (cdylib/Python) builds the cfg is false, so this is a no-op there; on a wasm build it fails the compile with a clear message instead of silently producing a component that exports nothing.
...
```rust
fn generate_descriptor (trait_name : & Ident , impl_ident : & Ident , methods : & [& Ident] , crate_path : & Path , buffer_strategy : BufferStrategyAttr , config : Option < & Path > ,) -> TokenStream
```
